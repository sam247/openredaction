...
function findPattern(
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].includes(`type: '${patternType}'`) ||
        lines[i].includes(`type: "${patternType}"`)
      ) {
...
function updateValidatorForSeparators(validatorCode: string): string {
  ...
  if (
    validatorCode.includes("normalizeSeparators") ||
    validatorCode.includes("replace(/[\\s")
  ) {
  ...
  for (const line of lines) {
    if (
      (!normalizedAdded && line.trim().startsWith("const")) ||
...
function rollback(): void {
  ...
  const backupFiles = execSync(
    `ls -t "${BACKUP_DIR}" | grep "${change.file.replace(/\//g, "_")}" | head -1`,
  )
    .toString()
    .trim();
...
function runTests(patternType?: string): void {
  ...
  try {
    if (patternType) {
      execSync(
        `cd packages/core && npm test -- pattern-hardening.test.ts -t "${patternType}"`,
        {
          stdio: "inherit",
        },
      );
