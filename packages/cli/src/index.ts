import { OpenRedaction, ConfigLoader } from "@openredaction/core";
...
async function main() {
  ...
  if (command === "stats") {
    ...
    if (whitelist.length > 0) {
      ...
      whitelist.slice(0, 10).forEach((entry) => {
        console.log(
          `  - "${entry.pattern}" (confidence: ${(entry.confidence * 100).toFixed(0)}%, occurrences: ${entry.occurrences})`,
        );
      });
  ...
  if (command === "feedback") {
    ...
    if (!feedbackText) {
      ...
      console.log(
        "Usage: openredact feedback <false-positive|false-negative> <text> [--type TYPE] [--context CONTEXT]",
      );
      process.exit(1);
    }
    ...
    if (!["false-positive", "false-negative"].includes(feedbackType)) {
      console.error(
        'Error: Feedback type must be "false-positive" or "false-negative"',
      );
      process.exit(1);
    }
    ...
    if (feedbackType === "false-positive") {
      ...
      const confidence =
        shield.getLearningStore()?.getConfidence(feedbackText) || 0;
      ...
      if (confidence >= 0.85) {
        ...
        console.log(
          `  Need ${Math.ceil((0.85 - confidence) * 20)} more occurrence(s) for auto-whitelist`,
        );
      }
    } else {
    ...
    if (jsonOutput) {
      ...
      if (result.detections.length === 0) {
        ...
        result.detections.forEach((detection, i) => {
          ...
          console.log(
            `   Position: [${detection.position[0]}, ${detection.position[1]}]\n`,
          );
        });
      }
