...
import { validatePattern, isUnsafePattern } from "@openredaction/core";
...
function validatePatternCommand(pattern: string, options: CLIOptions) {
  ...
  try {
    ...
    if (isUnsafePattern(pattern)) {
      result.safe = false;
      result.errors.push(
        "Pattern contains potentially unsafe constructs (ReDoS risk)",
      );
      ...
      if (/\([^)]*\|[^)]*\)[*+{]/.test(pattern)) {
        result.warnings.push(
          "Overlapping alternation with quantifier: (a|ab)+",
        );
      }
      if (/[*+?{][*+?{]/.test(pattern)) {
    ...
    if (pattern.length > 5000) {
      result.safe = false;
      result.errors.push(
        `Pattern too long: ${pattern.length} chars (max 5000)`,
      );
    }
    ...
    if (result.matchCount > 0) {
      ...
      result.matches.forEach((m: any, i: number) => {
        ...
        if (
          ...
          m.captureGroups.some((g: string) => g)
        ) {
...
function checkPatternCommand(pattern: string, options: CLIOptions) {
  ...
  try {
    ...
    if (!flags.includes("g") && /[*+{]/.test(pattern)) {
      result.warnings.push(
        "Pattern has quantifiers but no global flag - will only match once",
      );
    }
  } catch (error) {
    result.valid = false;
  ...
  if (options.json) {
    ...
    if (result.valid) {
      ...
      console.log(
        `  Has capture groups: ${result.info.hasGroups ? "Yes" : "No"}`,
      );
      console.log(
        `  Has quantifiers: ${result.info.hasQuantifiers ? "Yes" : "No"}`,
      );
      console.log(
        `  Has anchors (^/$): ${result.info.hasAnchors ? "Yes" : "No"}`,
      );
      console.log(
        `  Has lookahead: ${result.info.hasLookahead ? "Yes" : "No"}`,
      );
      console.log(
        `  Has lookbehind: ${result.info.hasLookbehind ? "Yes" : "No"}`,
      );
    }
...
function benchmarkPatternCommand(
  pattern: string,
  text: string,
  options: CLIOptions,
) {
  ...
  try {
    ...
    result.metrics = {
      ...
      matchCount,
      matchesPerMs:
        matchCount > 0 ? (matchCount / executionTime).toFixed(2) : "0",
    ...
    switch (command) {
      ...
      case "test": {
        ...
        if (!pattern || !text) {
          ...
          console.log(
            "Usage: openredaction-test-pattern test <pattern> <text>",
          );
          process.exit(1);
        }
      ...
      case "check": {
        ...
        if (!pattern) {
          console.error("Error: Pattern is required");
          console.log(
            "Usage: openredaction-test-pattern check <pattern> [--flags <flags>]",
          );
          process.exit(1);
        }
      ...
      case "benchmark": {
        ...
        if (!pattern || !text) {
          console.error("Error: Pattern and text are required");
          console.log(
            "Usage: openredaction-test-pattern benchmark <pattern> <text>",
          );
          process.exit(1);
        }
