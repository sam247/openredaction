...
describe('Redaction Modes', () => {
  describe('Placeholder Mode (Default)', () => {
    it('should use placeholder format [TYPE_ID]', () => {
      const shield = new OpenRedaction({ redactionMode: 'placeholder' });
      const result = shield.detect('Contact john@example.com or call 555-123-4567');
  ...
  describe('Mask Middle Mode', () => {
    it('should mask middle of email while preserving domain', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
      const result = shield.detect('Contact john@example.com');
    ...
    it('should mask middle digits of phone number', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
      const result = shield.detect('Call 555-123-4567');

      // Phone should show area code and last 4: 555-**-4567
      expect(result.redacted).toMatch(/555-\*+-4567/);
    });

    it('should mask SSN showing only last 4', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
      const result = shield.detect('SSN: 123-45-6789');

      // SSN should be: ***-**-6789
      expect(result.redacted).toContain('***-**-6789');
    });

    it('should mask middle of credit card showing first 4 and last 4', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
      const result = shield.detect('Card: 4532-1234-5678-9010');
  ...
  describe('Mask All Mode', () => {
    it('should replace entire value with asterisks', () => {
  ...
  describe('Format Preserving Mode', () => {
    it('should preserve structure with Xs', () => {
    ...
    it('should preserve email format with Xs', () => {
      const shield = new OpenRedaction({ redactionMode: 'format-preserving' });
      const result = shield.detect('Email: john@example.com');
  ...
  describe('Token Replace Mode', () => {
    it('should replace with realistic fake email', () => {
    ...
    it('should replace with fake phone number', () => {
      const shield = new OpenRedaction({ redactionMode: 'token-replace' });
      const result = shield.detect('Call 555-123-4567');
    ...
    it('should replace with fake SSN', () => {
      const shield = new OpenRedaction({ redactionMode: 'token-replace' });
      const result = shield.detect('SSN: 123-45-6789');
    ...
    it('should be deterministic for same value', () => {
      ...
      const result1 = shield.detect('Email: john@example.com and john@example.com again');
      const result2 = shield.detect('Email: john@example.com');
  ...
  describe('Multiple Values', () => {
    it('should handle multiple PII with consistent mode', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
  ...
  describe('Redaction Map', () => {
    it('should maintain redaction map regardless of mode', () => {
      const shield = new OpenRedaction({ redactionMode: 'mask-middle' });
      const result = shield.detect('Email: john@example.com');
