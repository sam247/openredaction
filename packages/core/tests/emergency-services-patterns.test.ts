...
describe("Emergency Services Pattern Detection", () => {
  describe("Emergency Call Reference", () => {
    it("should detect emergency call reference", async () => {
      ...
      const result = await detector.detect(
        "911 Call Reference: EMERGENCY-2024-123456",
      );
      expect(
        result.detections.some((d) => d.type === "EMERGENCY_CALL_REF"),
      ).toBe(true);
    });
  ...
  describe("Police Report Number", () => {
    it("should detect police report number", async () => {
      const detector = new OpenRedaction({
        patterns: ["POLICE_REPORT_NUMBER"],
      });
      const result = await detector.detect("Police Report: PR-2024-0012345");
      expect(
        result.detections.some((d) => d.type === "POLICE_REPORT_NUMBER"),
      ).toBe(true);
    });
  ...
  describe("Fire Incident Number", () => {
    it("should detect fire incident number", async () => {
      const detector = new OpenRedaction({
        patterns: ["FIRE_INCIDENT_NUMBER"],
      });
      const result = await detector.detect(
        "Fire Department incident: FI-2024-00123",
      );
      expect(
        result.detections.some((d) => d.type === "FIRE_INCIDENT_NUMBER"),
      ).toBe(true);
    });
  ...
  describe("Ambulance Call ID", () => {
    it("should detect ambulance call ID", async () => {
      ...
      expect(
        result.detections.some((d) => d.type === "AMBULANCE_CALL_ID"),
      ).toBe(true);
    });
  ...
  describe("Paramedic Certification", () => {
    it("should detect NREMT certification", async () => {
      const detector = new OpenRedaction({
        patterns: ["PARAMEDIC_CERTIFICATION"],
      });
      const result = await detector.detect(
        "NREMT-P certification: NREMT-P-123456",
      );
      expect(
        result.detections.some((d) => d.type === "PARAMEDIC_CERTIFICATION"),
      ).toBe(true);
    });
  ...
  describe("Emergency Shelter ID", () => {
    it("should detect emergency shelter registration", async () => {
      const detector = new OpenRedaction({
        patterns: ["EMERGENCY_SHELTER_ID"],
      });
      const result = await detector.detect(
        "Emergency shelter registration: SHELTER-A-12345",
      );
      expect(
        result.detections.some((d) => d.type === "EMERGENCY_SHELTER_ID"),
      ).toBe(true);
    });
    ...
    it("should detect evacuation ID", async () => {
      ...
      const result = await detector.detect(
        "Evacuation center: EVACUATION-2024-001",
      );
      expect(
        result.detections.some((d) => d.type === "EMERGENCY_SHELTER_ID"),
      ).toBe(true);
  ...
  describe("Disaster Victim ID", () => {
    it("should detect disaster victim identification", async () => {
      ...
      expect(
        result.detections.some((d) => d.type === "DISASTER_VICTIM_ID"),
      ).toBe(true);
    });
  ...
  describe("Search and Rescue Mission ID", () => {
    it("should detect SAR mission ID", async () => {
      const detector = new OpenRedaction({
        patterns: ["SEARCH_RESCUE_MISSION_ID"],
      });
      const result = await detector.detect("SAR mission: SAR-2024-001");
      expect(
        result.detections.some((d) => d.type === "SEARCH_RESCUE_MISSION_ID"),
      ).toBe(true);
    });
    ...
    it("should detect rescue operation", async () => {
      ...
      const result = await detector.detect(
        "Search and rescue: RESCUE-123456789",
      );
      expect(
        result.detections.some((d) => d.type === "SEARCH_RESCUE_MISSION_ID"),
      ).toBe(true);
  ...
  describe("Missing Person Case", () => {
    it("should detect missing person case number", async () => {
      ...
      const result = await detector.detect(
        "Missing person case: MP-2024-00123",
      );
      expect(
        result.detections.some((d) => d.type === "MISSING_PERSON_CASE"),
      ).toBe(true);
    });
  ...
  describe("Hazmat Incident", () => {
    it("should detect hazmat incident number", async () => {
      ...
      expect(result.detections.some((d) => d.type === "HAZMAT_INCIDENT")).toBe(
        true,
      );
    });
