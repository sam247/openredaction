...
describe("Explain API", () => {
  ...
  describe("Pattern matching details", () => {
    ...
    it("should include pattern details", async () => {
      ...
      const emailMatch = explanation.matchedPatterns.find(
        (r) => r.pattern.type === "EMAIL",
      );
  ...
  describe("Context analysis in explanations", () => {
    it("should include context analysis results", async () => {
      ...
      const matched = explanation.matchedPatterns.find(
        (r) => r.pattern.type === "EMAIL",
      );
    ...
    it("should explain low confidence filtering", async () => {
      ...
      const filtered = explanation.filteredPatterns.find(
        (r) => r.matchedValue === "test@example.com",
      );
  ...
  describe("Validation results", () => {
    it("should show validator failures", async () => {
      ...
      const failedValidation = explanation.filteredPatterns.some(
        (r) => r.validatorPassed === false,
      );
  ...
  describe("Explain specific detections", () => {
    it("should explain a specific detection", async () => {
      ...
      const detectionExplanation = await explainAPI.explainDetection(
        detection,
        text,
      );
  ...
  describe("Suggest why not detected", () => {
    ...
    it("should handle unknown types", async () => {
      ...
      const suggestion = await explainAPI.suggestWhy(
        "anything",
        "UNKNOWN_TYPE",
      );
      ...
      expect(suggestion.suggestions).toBeDefined();
      expect(
        suggestion.suggestions.some((s) => s.includes("No patterns found")),
      ).toBe(true);
    });
  });
  ...
  describe("Summary statistics", () => {
    it("should provide accurate summary", async () => {
      ...
      expect(explanation.summary.totalPatternsChecked).toBeGreaterThan(0);
      expect(explanation.summary.patternsMatched).toBeGreaterThanOrEqual(
        explanation.summary.finalDetections,
      );
      expect(explanation.summary.finalDetections).toBe(
    ...
    it("should count filtered patterns correctly", async () => {
      ...
      const totalMatched =
        explanation.matchedPatterns.length +
        explanation.filteredPatterns.length;
