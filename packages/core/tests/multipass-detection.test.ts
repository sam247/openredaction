  ...
  describe("Integration with OpenRedaction", () => {
    it("should work when multi-pass is disabled (default)", async () => {
      ...
      const result = await redactor.detect(
        "API Key: AKIA1234567890ABCDEF, Email: user@company.com",
      );
    ...
    it("should prioritize credentials in multi-pass mode", async () => {
      const redactor = new OpenRedaction({ enableMultiPass: true });
      const text =
        "GitHub token: ghp_1234567890abcdefghij1234567890abcdefgh and email user@company.com";
      const result = await redactor.detect(text);

      expect(result.detections.length).toBeGreaterThan(0);

      // Check that GitHub token is detected
      const hasGitHubToken = result.detections.some(
        (d) => d.type.includes("GITHUB") || d.type.includes("TOKEN"),
      );
    ...
    it("should maintain consistency between single and multi-pass", async () => {
      ...
      expect(resultSingle.detections.length).toBe(
        resultMulti.detections.length,
      );
