...
describe("Pattern Category Filtering", () => {
  ...
  it("should load only financial category patterns", async () => {
    ...
    expect(
      patternTypes.some((t) => t.includes("CREDIT_CARD") || t.includes("IBAN")),
    ).toBe(true);
  ...
  it("should improve performance by using fewer patterns", async () => {
    ...
    expect(shieldFiltered.getPatterns().length).toBeLessThan(
      shieldAll.getPatterns().length,
    );

    console.log(
      `All patterns: ${shieldAll.getPatterns().length}, time: ${timeAll.toFixed(2)}ms`,
    );
    console.log(
      `Filtered patterns: ${shieldFiltered.getPatterns().length}, time: ${timeFiltered.toFixed(2)}ms`,
    );
    console.log(
      `Performance improvement: ${(((timeAll - timeFiltered) / timeAll) * 100).toFixed(1)}%`,
    );
  });
