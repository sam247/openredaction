...
describe('Pattern Category Filtering', () => {
  it('should load only personal category patterns', () => {
  ...
  it('should load only financial category patterns', () => {
    const shield = new OpenRedaction({
  ...
  it('should load multiple categories', () => {
    const shield = new OpenRedaction({
  ...
  it('should improve performance by using fewer patterns', () => {
    const text = 'Email: john@example.com, IP: 192.168.1.1';

    // All patterns
    const shieldAll = new OpenRedaction();
    const startAll = performance.now();
    const resultAll = shieldAll.detect(text);
    ...
    const resultFiltered = shieldFiltered.detect(text);
  ...
  it('should prioritize categories over includeNames/includeEmails', () => {
  ...
  it('should work with custom patterns alongside categories', () => {
    const shield = new OpenRedaction({
      categories: ['personal'],
      customPatterns: [{
        type: 'CUSTOM_ID',
