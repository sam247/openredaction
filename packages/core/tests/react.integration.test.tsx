...
describe('React hooks integration', () => {
  ...
  describe('usePIIDetector', () => {
    it('debounces detection until interval elapses', async () => {
      const { result, rerender } = renderHook(
        ({ text }: { text: string }) => usePIIDetector(text, { ...demoOptions, debounce: 80 }),
        { initialProps: { text: '' } },
      );
      ...
      await waitFor(
        () => {
          expect(result.current.result).not.toBeNull();
          expect(result.current.result!.redacted).not.toContain('notify@acmecorp.io');
        },
        { timeout: 4000 },
      );
    });
  ...
  describe('useFormFieldValidator', () => {
    ...
    it('returns true when input empty', async () => {
      const { result } = renderHook(() => useFormFieldValidator({ ...demoOptions, failOnPII: true }));
  ...
  describe('useAutoRedact', () => {
    it('debounced detect populates redactedText', async () => {
      const { result } = renderHook(() => useAutoRedact({ ...demoOptions, debounce: 80 }));
