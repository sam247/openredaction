...
describe("False Positive Filter", () => {
  describe("Version numbers", () => {
    it("should detect version numbers as false positives", async () => {
      const result = isFalsePositive(
        "1.2.3",
        "PHONE",
  ...
  describe("Dates", () => {
    it("should detect dates as false positives for phone patterns", async () => {
      const result = isFalsePositive("01-02-2024", "PHONE", "Date: 01-02-2024");
      expect(result.isFalsePositive).toBe(true);
      expect(result.confidence).toBeGreaterThan(0.8);
    });

    it("should detect birth dates", async () => {
      const result = isFalsePositive("12/25/1990", "PHONE", "born 12/25/1990");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect ISO dates", async () => {
      const result = isFalsePositive(
        "2024-01-15",
        "PHONE",
  ...
  describe("IP addresses", () => {
    ...
    it("should detect IPs in network context", async () => {
  ...
  describe("Measurements", () => {
    it("should detect measurements with units", async () => {
      const result = isFalsePositive("150", "PHONE", "height 150 cm");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect weight measurements", async () => {
      const result = isFalsePositive("75", "NUMBER", "weight: 75 kg");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect dimensions", async () => {
  ...
  describe("Years", () => {
    it("should detect years 1900-2099", async () => {
      const result = isFalsePositive("2024", "PHONE", "in 2024");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect historical years", async () => {
  ...
  describe("Prices", () => {
    it("should detect price amounts", async () => {
      const result = isFalsePositive("99.99", "ACCOUNT", "price: $99.99");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect costs", async () => {
      const result = isFalsePositive(
        "1250.00",
        "NUMBER",
        "total cost: £1250.00",
  ...
  describe("Port numbers", () => {
    it("should detect common ports", async () => {
      const result = isFalsePositive("8080", "PHONE", "listening on port 8080");
      expect(result.isFalsePositive).toBe(true);
      expect(result.confidence).toBeGreaterThan(0.8);
    });

    it("should detect ports with port keyword", async () => {
  ...
  describe("Percentages", () => {
    it("should detect percentages with percent symbol", async () => {
      const result = isFalsePositive("75", "NUMBER", "completion at 75 %");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect decimal percentages with keyword", async () => {
      const result = isFalsePositive(
        "99.5",
        "PHONE",
        "accuracy at 99.5 percent ",
      );
      expect(result.isFalsePositive).toBe(true);
    });
  });
  ...
  describe("Technical placeholders", () => {
    it("should detect foo/bar/baz placeholders", async () => {
      const result = isFalsePositive("foo", "NAME", "function foo() {");
      expect(result.isFalsePositive).toBe(true);
      expect(result.confidence).toBeGreaterThan(0.8);
    });

    it("should detect example domains", async () => {
      const result = isFalsePositive(
        "test@example.com",
        "EMAIL",
    ...
    it("should detect code comments", async () => {
      const result = isFalsePositive(
        "John Smith",
        "NAME",
  ...
  describe("Test data", () => {
    it("should detect test emails", async () => {
      const result = isFalsePositive(
        "test@example.com",
        "EMAIL",
    ...
    it("should detect johndoe/janedoe", async () => {
      const result = isFalsePositive(
        "johndoe@test.com",
        "EMAIL",
    ...
    it("should detect xxx patterns in values", async () => {
      const result = isFalsePositive("xxxxx", "PHONE", "format: xxxxx");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect 000 patterns in phone numbers", async () => {
      const result = isFalsePositive(
        "000-000-0000",
        "PHONE",
  ...
  describe("Programming keywords", () => {
    it("should detect keywords as non-names", async () => {
      const result = isFalsePositive("Function", "NAME", "a Function");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect reserved words", async () => {
  ...
  describe("Common words after articles", () => {
    it('should detect words right after "the"', async () => {
      const result = isFalsePositive("Bank", "NAME", "go to the ");
      expect(result.isFalsePositive).toBe(true);
      expect(result.confidence).toBeGreaterThanOrEqual(0.5);
    });

    it('should detect words right after "a"', async () => {
  ...
  describe("Test account numbers", () => {
    it("should detect all-zeros", async () => {
      const result = isFalsePositive(
        "0000-0000-0000",
        "ACCOUNT",
    ...
    it("should detect repeating digits", async () => {
      const result = isFalsePositive("1111111111", "CARD", "card: 1111111111");
      expect(result.isFalsePositive).toBe(true);
    });

    it("should detect sequential numbers", async () => {
      const result = isFalsePositive(
        "0123456789",
        "ACCOUNT",
        "test: 0123456789",
  ...
  describe("Timestamps", () => {
    it("should detect Unix timestamps", async () => {
      const result = isFalsePositive(
        "1234567890",
        "PHONE",
    ...
    it("should detect epoch times", async () => {
  ...
  describe("Real PII should not be flagged", () => {
    it("should not flag real phone numbers", async () => {
      const result = isFalsePositive(
        "07700 900123",
        "PHONE_UK_MOBILE",
        "Call me at 07700 900123",
      );
      expect(result.isFalsePositive).toBe(false);
    });

    it("should not flag real emails", async () => {
      const result = isFalsePositive(
    ...
    it("should not flag real account numbers", async () => {
      const result = isFalsePositive(
        "12345678",
        "UK_BANK_ACCOUNT",
        "Account: 12345678",
  ...
  describe("Integration with OpenRedact", () => {
    it("should filter version numbers when enabled", async () => {
      const redactor = new OpenRedaction({
        enableFalsePositiveFilter: true,
      });
      const result = await redactor.detect("App version 1.2.3 released");

      // Should not detect version as phone number
      const phoneDetections = result.detections.filter((d) =>
        d.type.includes("PHONE"),
      );
      expect(phoneDetections.length).toBe(0);
    });

    it("should filter test emails when enabled", async () => {
      const redactor = new OpenRedaction({
        enableFalsePositiveFilter: true,
      });
    ...
    it("should filter dates mistaken for phones when enabled", async () => {
      const redactor = new OpenRedaction({
        enableFalsePositiveFilter: true,
      });
      const result = await redactor.detect("Born on 01-02-1990");
    ...
    it("should not filter by default (disabled)", async () => {
      const redactor = new OpenRedaction();
      ...
      const emailDetections = result.detections.filter(
        (d) => d.type === "EMAIL",
      );
    ...
    it("should still detect real PII with filter enabled", async () => {
      const redactor = new OpenRedaction({
        enableFalsePositiveFilter: true,
      });
      const result = await redactor.detect(
        "Call John at 07700-900-123 for details",
      );
