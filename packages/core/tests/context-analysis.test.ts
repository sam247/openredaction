...
describe("Context Analysis", () => {
  describe("extractContext", () => {
    it("should extract words before and after detection", () => {
      const text =
        "Please contact John Doe at john@example.com for more information";
  ...
  describe("analyzeContextFeatures", () => {
    it("should detect technical context", () => {
      const context =
        "API endpoint at https://api.server.com returns JSON data";
    ...
    it("should detect business context", () => {
      const context =
        "Company CEO John Smith leads the team at Corporation Inc";
      const features = analyzeContextFeatures(context);
    ...
    it("should not penalize CREDIT_CARD when hasExampleContext is true (Issue #26)", () => {
      const confidence = calculateContextConfidence(
        ...
        "CREDIT_CARD",
        {
          ...
          after: "です。",
          sentence:
            "メールは taro@example.com でカード番号は 4111-1111-1111-1111 です。",
          documentType: "document",
          features: {
            hasTechnicalContext: false,
            hasBusinessContext: false,
  ...
  describe("analyzeFullContext", () => {
    it("should perform complete context analysis", () => {
      ...
      const analysis = analyzeFullContext(
        text,
        value,
        "PHONE",
        startPos,
  ...
  describe("Integration with OpenRedaction", () => {
    ...
    it("should filter low-confidence detections when enabled", async () => {
      ...
      const result = await redactor.detect(
        "This is a sample test with test@example.com dummy data",
      );
    ...
    it("should include confidence scores when enabled", async () => {
      ...
      const result = await redactor.detect(
        "Contact john@example.com for information",
      );
    ...
    it("should pass high-confidence detections", async () => {
      ...
      const result = await redactor.detect(
        "Dear Mr. Smith, please call me at 020-1234-5678",
      );
