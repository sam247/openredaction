...
describe("Digital Identity Pattern Detection", () => {
  describe("Discord User ID", () => {
    it("should detect valid Discord snowflake ID", async () => {
      ...
      expect(result.detections.some((d) => d.type === "DISCORD_USER_ID")).toBe(
        true,
      );
    });
  ...
  describe("Steam ID64", () => {
    ...
    it("should require 765 prefix", async () => {
      ...
      expect(result.detections.some((d) => d.type === "STEAM_ID64")).toBe(
        false,
      );
    });
  ...
  describe("Social Media Handle", () => {
    it("should detect Twitter handle", async () => {
      ...
      expect(
        result.detections.some((d) => d.type === "SOCIAL_MEDIA_HANDLE"),
      ).toBe(true);
    });
  ...
  describe("Twitter User ID", () => {
    it("should detect Twitter user ID", async () => {
      ...
      const result = await detector.detect(
        "Twitter user ID: 12345678901234567",
      );
      expect(result.detections.some((d) => d.type === "TWITTER_USER_ID")).toBe(
        true,
      );
    });
  ...
  describe("Facebook ID", () => {
    it("should detect Facebook profile ID", async () => {
      ...
      expect(result.detections.some((d) => d.type === "FACEBOOK_ID")).toBe(
        true,
      );
    });
  ...
  describe("TikTok Username", () => {
    it("should detect TikTok username", async () => {
      ...
      expect(result.detections.some((d) => d.type === "TIKTOK_USERNAME")).toBe(
        true,
      );
    });
  ...
  describe("LinkedIn Profile", () => {
    it("should detect LinkedIn profile URL", async () => {
      ...
      const result = await detector.detect(
        "LinkedIn profile: /in/john-smith-123456/",
      );
      expect(result.detections.some((d) => d.type === "LINKEDIN_PROFILE")).toBe(
        true,
      );
    });
  ...
  describe("YouTube Channel ID", () => {
    it("should detect YouTube channel ID", async () => {
      ...
      expect(
        result.detections.some((d) => d.type === "YOUTUBE_CHANNEL_ID"),
      ).toBe(true);
    });
  ...
  describe("Reddit Username", () => {
    it("should detect Reddit username", async () => {
      ...
      expect(result.detections.some((d) => d.type === "REDDIT_USERNAME")).toBe(
        true,
      );
    });
  ...
  describe("Nintendo Friend Code", () => {
    it("should detect Nintendo Switch Friend Code", async () => {
      const detector = new OpenRedaction({
        patterns: ["NINTENDO_FRIEND_CODE"],
      });
      const result = await detector.detect(
        "Nintendo Switch: SW-1234-5678-9012",
      );
      expect(
        result.detections.some((d) => d.type === "NINTENDO_FRIEND_CODE"),
      ).toBe(true);
    });
  ...
  describe("Xbox Gamertag", () => {
    it("should detect Xbox Gamertag", async () => {
      ...
      expect(result.detections.some((d) => d.type === "XBOX_GAMERTAG")).toBe(
        true,
      );
    });
  ...
  describe("Telegram User ID", () => {
    it("should detect Telegram user ID", async () => {
      ...
      expect(result.detections.some((d) => d.type === "TELEGRAM_USER_ID")).toBe(
        true,
      );
    });
  ...
  describe("Integration: Multiple Digital Identities", () => {
    it("should detect multiple digital identity patterns", async () => {
      ...
      const nintendoDetected = result.detections.some(
        (d) => d.type === "NINTENDO_FRIEND_CODE",
      );
