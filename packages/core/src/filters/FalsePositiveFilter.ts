...
export const commonFalsePositives: FalsePositiveRule[] = [
  ...
  // Dates that look like phone numbers
  {
    ...
    matcher: (value: string, context: string) => {
      ...
      const dateContext =
        /\b(date|born|birth|dob|created|updated|on|since|until|before|after)\s*[:\s]*/i;
  ...
  // Measurements and dimensions
  {
    ...
    matcher: (value: string, context: string) => {
      // Unit indicators
      const unitContext =
        /\b(cm|mm|km|m|ft|in|inch|meter|mile|kg|lb|oz|gram|litre|liter|ml|gb|mb|kb)\s*$/i;
      const hasUnit = unitContext.test(context + " " + value);

      // Measurement context
      const measureContext =
        /\b(size|width|height|length|weight|distance|volume|capacity|dimension)\s*[:\s]*/i;
  ...
  // Prices and monetary amounts
  {
    ...
    matcher: (value: string, context: string) => {
      ...
      const currencyContext =
        /\b(price|cost|amount|total|subtotal|fee|charge|payment|\$|£|€|¥|USD|GBP|EUR)\s*[:\s]*/i;
  ...
  // Codes and identifiers in technical documentation
  {
    ...
    matcher: (value: string, context: string) => {
      ...
      const techPlaceholders =
        /\b(foo|bar|baz|qux|example|test|demo|sample|placeholder|dummy|mock)\b/i;
  ...
  // Timestamps
  {
    ...
    matcher: (_value: string, context: string) => {
      const timestampContext =
        /\b(timestamp|time|epoch|unix|millis|seconds|created.at|updated.at)\s*[:\s]*/i;
...
export function isFalsePositive(
  ...
  for (const rule of rules) {
    ...
    if (rule.matcher(value, context)) {
      const confidence =
        rule.severity === "high" ? 0.9 : rule.severity === "medium" ? 0.7 : 0.5;
      return {
