...
export const personalPatterns: PIIPattern[] = [
  {
    ...
    validator: (value: string, context: string) => {
      ...
      const isLegitimateTest =
        /test|sample|demo|spec|api|reference|guide|template|documentation/i.test(
          context,
        );
      ...
      const testDomains =
        /@test\.com|@example\.com|@sample\.com|@demo\.com|@fake\.com|@placeholder\.com/i;
      if (testDomains.test(value)) {
        const legitimateTestContext =
          /test|spec|api|reference|guide|template|documentation|john\+|!!!|\+tag|john@/i.test(
            context + value,
          );
  ...
  },
  {
    ...
    regex:
      /\b(?:(?:Mr|Mrs|Ms|Miss|Dr|Prof|Professor|Sir|Madam|Lady|Lord|Rev|Father|Sister|Brother)\.?\s+)?((?:[A-Z][a-z'’.-]+|[A-Z]{2,})(?:\s+(?:[A-Z][a-z'’.-]+|[A-Z]{2,}|[a-z][a-z'’.-]+)){1,3})(?:\s+(?:Jr|Sr|II|III|IV|PhD|MD|Esq|DDS|DVM|MBA|CPA)\.?)?\b/g,
    priority: 50,
    validator: (value: string, context: string) => {
      ...
      const rejectKeywords =
        /example|test|sample|demo|fake|placeholder|john\s+doe|jane\s+smith/i;
      ...
      const businessTerms =
        /\b(company|corporation|inc|llc|ltd|corp|organization|business|enterprise|firm|agency)\b/i;
    ...
    placeholder: "[NAME_{n}]",
    description:
      "Person name with salutations/suffixes (handles case variations)",
    severity: "high",
  },
  {
    type: "EMPLOYEE_ID",
    regex:
      /\b(?:EMP|EMPL|EMPLOYEE)[_\s-]?(?:ID|NUM(?:BER)?)?[:\s-]*([A-Z0-9]{4,10})\b/gi,
    priority: 90,
  ...
  },
  {
    type: "DATE_OF_BIRTH",
    regex:
      /\b(?:DOB|date of birth|birth ?date)[:\s-]*((?:\d{1,2}[/\-.]\d{1,2}[/\-.]\d{2,4})|(?:\d{1,2}\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{2,4}))\b/gi,
    priority: 95,
    placeholder: "[DOB_{n}]",
