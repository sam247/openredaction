...
export const contactPatterns: PIIPattern[] = [
  {
    ...
    regex:
      /\b(?:\+?44[\s\u00A0.-]?7\d{3}|0?7\d{3})[\s\u00A0.-]?\d{3}[\s\u00A0.-]?\d{3}\b/g,
    priority: 90,
    ...
    validator: (value: string, context: string) => {
      ...
      const strongRejectKeywords =
        /example\s+phone|test\s+number|sample\s+phone|demo\s+phone/i;
  ...
  },
  {
    type: "PHONE_UK",
    regex:
      /\b(?:\+?44[\s\u00A0.-]?(?:0)?\s*)?(?:\(?0?[1-9]\d{1,3}\)?[\s\u00A0.-]?\d{3,4}[\s\u00A0.-]?\d{3,4})(?:\s?(?:ext\.?|x)\s?\d{1,5})?\b/g,
    priority: 85,
  ...
  },
  {
    type: "PHONE_US",
    regex:
      /\b(?:\+1[\s\u00A0.-]?)?(?:\(\d{3}\)|\d{3})[\s\u00A0.-]?\d{3}[\s\u00A0.-]?\d{4}(?:\s?(?:ext\.?|x)\s?\d{1,6})?\b/g,
    priority: 85,
    ...
    validator: (value: string, context: string) => {
      ...
      if (areaCode === "555") {
        ...
        if (
          /example\s+phone|test\s+number|fictional\s+number|demo\s+phone/i.test(
            contextLower,
          )
        ) {
          return false;
  ...
  },
  {
    ...
    regex:
      /\b\+(?:\d[\s\u00A0.\-()]?){6,14}\d(?:\s?(?:ext\.?|x)\s?\d{1,6})?\b/g,
    priority: 80,
  ...
  },
  {
    ...
    validator: (value: string, context: string) => {
      ...
      const contextLower = context.toLowerCase();
      if (
        /\b(phone|tel|call|contact)\b/i.test(contextLower) &&
        cleaned.length === 9
      ) {
  ...
  },
  {
    type: "ADDRESS_STREET",
    regex:
      /\b\d{1,5}\s+[A-Za-z0-9][A-Za-z0-9'’.-]*(?:\s+[A-Za-z0-9][A-Za-z0-9'’.-]*){0,4}\s+(?:Street|St\.?|Road|Rd\.?|Avenue|Ave\.?|Lane|Ln\.?|Drive|Dr\.?|Court|Ct\.?|Boulevard|Blvd\.?|Way|Terrace|Ter\.?|Place|Pl\.?|Trail|Trl\.?|Parkway|Pkwy\.?|Highway|Hwy\.)(?:\s+(?:Apt|Unit|Suite|Ste)\s*\d+)?\b/gi,
    priority: 70,
