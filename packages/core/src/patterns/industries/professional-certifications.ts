...
export const PMP_CERTIFICATION: PIIPattern = {
  ...
  regex:
    /\bPMP[-\s]?(?:ID|NO|NUM|NUMBER|CERT(?:IFICATION)?)?[-\s]?[:#]?\s*(\d{7,9})\b/gi,
  placeholder: "[PMP_{n}]",
  priority: 80,
  severity: "medium",
  ...
  validator: (value: string, context: string) => {
    ...
    return /pmp|project[- ]?management|pmi|certification|certified/i.test(
      context,
    );
  },
};
...
export const CPA_LICENSE: PIIPattern = {
  type: "CPA_LICENSE",
  regex:
    /\bCPA[-\s]?(?:LICENSE|LIC|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{5,10})\b/gi,
  placeholder: "[CPA_{n}]",
  priority: 85,
  severity: "medium",
  description: "CPA (Certified Public Accountant) license number",
  validator: (_value: string, context: string) => {
    return /cpa|certified[- ]?public[- ]?accountant|accountancy|license|accounting/i.test(
      context,
    );
  },
};
...
export const PE_LICENSE: PIIPattern = {
  type: "PE_LICENSE",
  regex:
    /\bPE[-\s]?(?:LICENSE|LIC|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{5,10})\b/gi,
  placeholder: "[PE_{n}]",
  priority: 80,
  severity: "medium",
  description: "PE (Professional Engineer) license number",
  validator: (_value: string, context: string) => {
    return /professional[- ]?engineer|engineering|pe[- ]?license|registered[- ]?engineer/i.test(
      context,
    );
  },
};
...
export const NURSING_LICENSE: PIIPattern = {
  ...
  regex:
    /\b(?:RN|LPN|NP|NURSING)[-\s]?(?:LICENSE|LIC|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[RN_{n}]",
  priority: 85,
  severity: "high",
  description: "Nursing license number (RN, LPN, NP)",
  validator: (_value: string, context: string) => {
    return /nurse|nursing|rn|lpn|registered[- ]?nurse|license|practitioner/i.test(
      context,
    );
  },
};
...
export const TEACHING_LICENSE: PIIPattern = {
  ...
  regex:
    /\b(?:TEACHING|TEACHER|EDUCATOR)[-\s]?(?:LICENSE|LIC|CERT(?:IFICATE)?|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[TEACHER_{n}]",
  priority: 80,
  severity: "medium",
  description: "Teaching certificate/license number",
  validator: (_value: string, context: string) => {
    return /teacher|teaching|educator|education|certificate|license|certified/i.test(
      context,
    );
  },
};
...
export const AWS_CERTIFICATION: PIIPattern = {
  ...
  regex:
    /\bAWS[-\s]?(?:CERT(?:IFICATION)?|ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[AWS_CERT_{n}]",
  priority: 75,
  severity: "low",
  description: "AWS (Amazon Web Services) certification ID",
  validator: (_value: string, context: string) => {
    return /aws|amazon[- ]?web[- ]?services|cloud|certification|certified|solutions[- ]?architect/i.test(
      context,
    );
  },
};
...
export const MICROSOFT_CERTIFICATION: PIIPattern = {
  ...
  regex:
    /\b(?:MICROSOFT|MCID|MS)[-\s]?(?:CERT(?:IFICATION)?|ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[MS_CERT_{n}]",
  priority: 75,
  severity: "low",
  description: "Microsoft certification ID (MCID)",
  validator: (_value: string, context: string) => {
    return /microsoft|mcid|azure|certification|certified|mcsa|mcse/i.test(
      context,
    );
  },
};
...
export const CISCO_CERTIFICATION: PIIPattern = {
  ...
  regex:
    /\b(?:CISCO|CSCO)[-\s]?(?:CERT(?:IFICATION)?|ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[CISCO_CERT_{n}]",
  priority: 75,
  severity: "low",
  description: "Cisco certification ID",
  validator: (_value: string, context: string) => {
    return /cisco|ccna|ccnp|ccie|networking|certification|certified/i.test(
      context,
    );
  },
};
...
export const COMPTIA_CERTIFICATION: PIIPattern = {
  ...
  regex:
    /\bCOMPTIA[-\s]?(?:CERT(?:IFICATION)?|ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[COMPTIA_{n}]",
  priority: 75,
  severity: "low",
  description: "CompTIA certification ID",
  validator: (_value: string, context: string) => {
    return /comptia|a\+|security\+|network\+|certification|certified/i.test(
      context,
    );
  },
};
...
export const FINRA_LICENSE: PIIPattern = {
  type: "FINRA_LICENSE",
  regex:
    /\b(?:CRD|SERIES|FINRA)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*(\d{6,8})\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return /finra|crd|series|broker|dealer|securities|license|registered/i.test(
      context,
    );
  },
};
