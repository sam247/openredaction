...
export const MEDICAL_RECORD_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:MR[N]?[-\s]?|MEDICAL[-\s]?REC(?:ORD)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*)([A-Z0-9]{6,12})\b/gi,
  placeholder: "[MRN_{n}]",
  priority: 85,
  severity: "high",
...
export const PATIENT_ID: PIIPattern = {
  type: "PATIENT_ID",
  regex:
    /\b(?:PATIENT[-\s]?(?:ID|NUM(?:BER)?|REF(?:ERENCE)?)[-\s]?[:#]?\s*)([A-Z0-9]{6,12})\b/gi,
...
export const APPOINTMENT_REF: PIIPattern = {
  ...
  regex:
    /\b(?:APT|APPT|APPOINTMENT)[-\s]?(?:REF|ID|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,10})\b/gi,
  placeholder: "[APT_{n}]",
  priority: 75,
  severity: "medium",
...
export const ICD10_CODE: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const medicalContext = /diagnos|condition|disease|disorder|icd|code/i.test(
      context,
    );
...
export const CPT_CODE: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const medicalContext = /procedure|cpt|billing|treatment|service/i.test(
      context,
    );
...
export const PRESCRIPTION_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:RX|PRESC(?:RIPTION)?|SCRIPT)[-\s]?(?:NO|NUM(?:BER)?|REF|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[RX_{n}]",
  priority: 80,
  severity: "high",
...
export const HEALTH_INSURANCE_CLAIM: PIIPattern = {
  ...
  regex:
    /\b(?:CLAIM|CLM)[-\s]?(?:NO|NUM(?:BER)?|REF|ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
...
export const HEALTH_PLAN_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:HEALTH[-\s]?PLAN|BENEFICIARY|MEMBER)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,15})\b/gi,
...
export const MEDICAL_DEVICE_SERIAL: PIIPattern = {
  ...
  regex:
    /\b(?:DEVICE|IMPLANT|PACEMAKER|DEFIBRILLATOR)[-\s]?(?:SERIAL|SN|S\/N)[-\s]?[:#]?\s*([A-Z0-9]{8,20})\b/gi,
...
export const LAB_TEST_ID: PIIPattern = {
  type: "LAB_TEST_ID",
  regex:
    /\b(?:LAB|TEST|SAMPLE)[-\s]?(?:ID|NUM(?:BER)?|REF)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[LAB_{n}]",
  priority: 75,
  severity: "medium",
...
export const TRIAL_PARTICIPANT_ID: PIIPattern = {
  ...
  regex:
    /\b(?:PARTICIPANT|SUBJECT|TRIAL)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{1,2}[-]?\d{4,6})\b/gi,
...
export const PROTOCOL_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:PROTOCOL|STUDY)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
...
export const BIOBANK_SAMPLE_ID: PIIPattern = {
  ...
  regex:
    /\b(?:BIOBANK|SAMPLE|SPECIMEN)[-\s]?(?:ID|NO)?[-\s]?[:#]?\s*([A-Z0-9]{8,15})\b/gi,
...
export const NPI_NUMBER: PIIPattern = {
  type: "NPI_NUMBER",
  regex:
    /\b(?:NPI[-\s\u00A0]*(?:NO|NUM(?:BER)?)?[-\s\u00A0.:#]*)?((?:\d[\s\u00A0.-]?){10})\b/g,
  placeholder: "[NPI_{n}]",
  priority: 85,
  severity: "high",
  ...
  validator: (value: string, context: string) => {
    ...
    if (
      !/provider|npi|physician|doctor|clinic|hospital|practice/i.test(context)
    ) {
      return false;
    }
...
export const DEA_NUMBER: PIIPattern = {
  type: "DEA_NUMBER",
  regex:
    /\b(?:DEA[-\s\u00A0]*(?:NO|NUM(?:BER)?)?[-\s\u00A0.:#]*)?([A-Z]{2}(?:[\s\u00A0.-]?\d){7})\b/gi,
  placeholder: "[DEA_{n}]",
  priority: 90,
  severity: "high",
...
export const HOSPITAL_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\b(?:HOSPITAL|H|HAR)[-\s]?(?:ACCOUNT|ACCT|A\/C)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const EMERGENCY_CONTACT_MARKER: PIIPattern = {
  ...
  regex:
    /(?:emergency\s+contact|next\s+of\s+kin|ice|in\s+case\s+of\s+emergency)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3})/gi,
...
export const DNA_SEQUENCE: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const geneticContext = /dna|genetic|sequence|genome|nucleotide|gene/i.test(
      context,
    );
...
export const DRUG_DOSAGE: PIIPattern = {
  type: "DRUG_DOSAGE",
  regex:
    /\b([A-Z][a-z]+(?:ine|ol|azole|mycin|cillin|pril|olol|mab|pam|tab|pine|done|ide|tide|ase|statin))\s+(\d+(?:\.\d+)?)\s?(mg|mcg|g|ml|units?|IU)\b/gi,
...
export const ALLERGY_INFO: PIIPattern = {
  type: "ALLERGY_INFO",
  regex:
    /\b(?:allergic\s+to|allergy)[:\s]+([A-Za-z\s,]+(?:penicillin|peanuts|latex|aspirin|shellfish|eggs|dairy|soy|wheat))/gi,
...
export const VACCINATION_ID: PIIPattern = {
  type: "VACCINATION_ID",
  regex:
    /\b(?:VACCINE|VACCINATION|IMMUNIZATION)[-\s]?(?:ID|RECORD|NO)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[VAX_{n}]",
  priority: 80,
  severity: "high",
...
export const CHI_NUMBER: PIIPattern = {
  type: "CHI_NUMBER",
  regex:
    /\b(?:CHI|community health index)[-\s]?(?:number|no)?[-\s]?[:#]?\s*(\d{6}[-\s]?\d{4})\b/gi,
  placeholder: "[CHI_{n}]",
  priority: 95,
  severity: "high",
  ...
  validator: (match) => {
    ...
    const expectedCheckDigit =
      checkDigit === 11 ? 0 : checkDigit === 10 ? 0 : checkDigit;
...
export const EHIC_NUMBER: PIIPattern = {
  type: "EHIC_NUMBER",
  regex:
    /\b(?:EHIC|european health insurance|health card)[-\s]?(?:number|no)?[-\s]?[:#]?\s*([A-Z]{2}\s?\d{12,16})\b/gi,
  placeholder: "[EHIC_{n}]",
  priority: 90,
  severity: "high",
