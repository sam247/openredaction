...
export const DONOR_ID: PIIPattern = {
  ...
  validator: (_value: string, context: string) => {
    return /donor|donation|charitable|contribution|gift|philanthrop/i.test(
      context,
    );
  },
};
...
export const UK_CHARITY_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:Charity\s+(?:No|Number|Registration|Reg)\.?\s*:?\s*)?(\d{6,7}(?:-\d)?)\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return (
      digits.length >= 6 &&
      digits.length <= 8 &&
      /charity|charitable|commission/i.test(context)
    );
  },
};
...
export const US_EIN: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    return (
      digits.length === 9 &&
      /nonprofit|charity|501\(c\)|tax[-\s]exempt|foundation/i.test(context)
    );
  },
};
