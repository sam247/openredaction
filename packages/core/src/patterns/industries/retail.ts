...
export const ORDER_NUMBER: PIIPattern = {
  type: "ORDER_NUMBER",
  regex:
    /\b(?:ORD(?:ER)?[-\s](?:NO|NUM(?:BER)?)?[-\s:#]?\s*|ORDER\s+(?:NO|NUM(?:BER)?)?[:\s]+)([A-Z0-9-]{8,14})\b/gi,
...
export const LOYALTY_CARD_NUMBER: PIIPattern = {
  ...
  regex:
    /\bLOYALTY[-\s]?(?:CARD)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{10,16})\b/gi,
...
export const CUSTOMER_ID: PIIPattern = {
  type: "CUSTOMER_ID",
  regex:
    /\b(?:CUSTOMER|CUST)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const GIFT_CARD_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:GIFT[-\s]?CARD|GC)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{12,19})\b/gi,
...
export const RMA_NUMBER: PIIPattern = {
  type: "RMA_NUMBER",
  regex:
    /\b(?:RMA|RETURN)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
  placeholder: "[RMA_{n}]",
  priority: 80,
  severity: "medium",
...
export const SHIPPING_TRACKING: PIIPattern = {
  ...
  regex:
    /\b(?:TRACKING|TRACK)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{10,30})\b/gi,
...
export const INVOICE_NUMBER: PIIPattern = {
  type: "INVOICE_NUMBER",
  regex:
    /\b(?:INVOICE|INV)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const PROMO_CODE: PIIPattern = {
  type: "PROMO_CODE",
  regex:
    /\b(?:PROMO|COUPON|DISCOUNT)[-\s]?(?:CODE)?[-\s]?[:#]?\s*([A-Z0-9]{6,16})\b/gi,
...
export const WISHLIST_ID: PIIPattern = {
  type: "WISHLIST_ID",
  regex:
    /\b(?:WISHLIST|WISH[-\s]?LIST)[-\s]?(?:ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
