...
export const EMPLOYEE_ID: PIIPattern = {
  type: "EMPLOYEE_ID",
  regex:
    /\b(?:EMPLOYEE|EMP|STAFF|PERSONNEL|WORKER)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{0,2}\d{4,10})\b/gi,
...
export const PAYROLL_NUMBER: PIIPattern = {
  type: "PAYROLL_NUMBER",
  regex:
    /\b(?:PAYROLL|PAY)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const SALARY_AMOUNT: PIIPattern = {
  type: "SALARY_AMOUNT",
  regex:
    /\b(?:SALARY|COMPENSATION|PAY|WAGE|EARNING)[-\s]?[:#]?\s*(?:[$£€¥]\s?)?(\d{1,3}(?:[,\s]\d{3})*(?:\.\d{2})?)\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    const compContext =
      /salary|compensation|pay|wage|earning|income|annual|hourly/i.test(
        context,
      );
...
export const PERFORMANCE_REVIEW_ID: PIIPattern = {
  ...
  regex:
    /\b(?:PERFORMANCE|REVIEW|APPRAISAL|EVALUATION)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[REVIEW_{n}]",
  priority: 80,
  severity: "high",
  description: "Performance review and appraisal identifiers",
  validator: (_value: string, context: string) => {
    return /performance|review|appraisal|evaluation|rating|assessment/i.test(
      context,
    );
  },
};
...
export const JOB_APPLICATION_ID: PIIPattern = {
  ...
  regex:
    /\b(?:APPLICATION|CANDIDATE|APPLICANT)[-\s]?(?:ID|NO|NUM(?:BER)?|REF)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
  placeholder: "[APPLICATION_{n}]",
  priority: 85,
  severity: "high",
  description: "Job application and candidate identifiers",
  validator: (_value: string, context: string) => {
    return /application|candidate|applicant|job|position|recruit|hiring/i.test(
      context,
    );
  },
};
...
export const RESUME_ID: PIIPattern = {
  type: "RESUME_ID",
  regex:
    /\b(?:RESUME|CV|CURRICULUM[-\s]?VITAE)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const BENEFITS_PLAN_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:BENEFITS?|INSURANCE|HEALTH[-\s\u00A0]?PLAN)[-\s\u00A0]*(?:PLAN)?[-\s\u00A0]*(?:NO|NUM(?:BER)?|ID)?[-\s\u00A0.:#]*([A-Z0-9](?:[A-Z0-9][\s\u00A0./-]?){5,15}[A-Z0-9])\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    const inContext =
      /benefit|insurance|health|dental|vision|plan|policy|enrollment/i.test(
        context,
      );
...
export const RETIREMENT_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\b(?:401K|403B|IRA|RETIREMENT|PENSION)[-\s]?(?:ACCOUNT)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[RETIREMENT_{n}]",
  priority: 90,
  severity: "high",
  description: "401(k), IRA, and retirement account numbers",
  validator: (_value: string, context: string) => {
    return /401k|403b|ira|retirement|pension|fund|invest|contribution/i.test(
      context,
    );
  },
};
...
export const DIRECT_DEPOSIT_REF: PIIPattern = {
  ...
  regex:
    /\b(?:DIRECT[-\s]?DEPOSIT|DD|ROUTING)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*(\d{9})\b/g,
  placeholder: "[DD_{n}]",
  priority: 90,
  severity: "high",
  description: "Direct deposit and routing numbers",
  validator: (_value: string, context: string) => {
    return /direct[-\s]?deposit|routing|bank|account|payroll|pay/i.test(
      context,
    );
  },
};
...
export const BACKGROUND_CHECK_ID: PIIPattern = {
  ...
  regex:
    /\b(?:BACKGROUND[-\s]?CHECK|BGC|SCREENING)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
  placeholder: "[BGC_{n}]",
  priority: 85,
  severity: "high",
...
export const DRUG_TEST_ID: PIIPattern = {
  type: "DRUG_TEST_ID",
  regex:
    /\b(?:DRUG[-\s]?TEST|SCREENING|URINALYSIS)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const TIMESHEET_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:TIMESHEET|TIMECARD|TIME[-\s]?ENTRY)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const TRAINING_CERT_ID: PIIPattern = {
  ...
  regex:
    /\b(?:TRAINING|CERTIFICATION|CERT)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[TRAINING_{n}]",
  priority: 70,
  severity: "medium",
  description: "Training and certification identifiers",
  validator: (_value: string, context: string) => {
    return /training|certification|cert|course|completion|credential/i.test(
      context,
    );
  },
};
...
export const EXPENSE_REPORT_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:EXPENSE|REIMBURSEMENT)[-\s]?(?:REPORT)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const LEAVE_REQUEST_ID: PIIPattern = {
  ...
  regex:
    /\b(?:PTO|LEAVE|VACATION|TIME[-\s]?OFF)[-\s]?(?:REQUEST)?[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[LEAVE_{n}]",
  priority: 75,
  severity: "medium",
  description: "Leave request and time-off identifiers",
  validator: (_value: string, context: string) => {
    return /pto|leave|vacation|time[-\s]?off|absence|sick|personal/i.test(
      context,
    );
  },
};
...
export const EXIT_INTERVIEW_ID: PIIPattern = {
  ...
  regex:
    /\b(?:EXIT|TERMINATION|SEPARATION)[-\s]?(?:INTERVIEW)?[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[EXIT_{n}]",
  priority: 80,
  severity: "high",
  description: "Exit interview and termination identifiers",
  validator: (_value: string, context: string) => {
    return /exit|termination|separation|offboard|departure|resign/i.test(
      context,
    );
  },
};
...
export const DISCIPLINARY_ACTION_ID: PIIPattern = {
  ...
  regex:
    /\b(?:DISCIPLINARY|INCIDENT|WARNING|VIOLATION)[-\s\u00A0]*(?:ACTION)?[-\s\u00A0]*(?:NO|NUM(?:BER)?|ID)?[-\s\u00A0.:#]*([A-Z0-9](?:[A-Z0-9][\s\u00A0./-]?){5,15}[A-Z0-9])\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return (
      hasDigits &&
      validLength &&
      /disciplinary|incident|warning|violation|misconduct|investigation/i.test(
        context,
      )
    );
  },
...
export const EMERGENCY_CONTACT_REF: PIIPattern = {
  ...
  regex:
    /\b(?:EMERGENCY[-\s]?CONTACT|ICE)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const WORK_PERMIT: PIIPattern = {
  type: "WORK_PERMIT",
  regex:
    /\b(?:WORK[-\s]?PERMIT|VISA|H1B|GREEN[-\s]?CARD|EAD)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,15})\b/gi,
...
export const SECURITY_CLEARANCE: PIIPattern = {
  ...
  regex:
    /\b(?:CLEARANCE|SECURITY[-\s]?LEVEL)[-\s]?[:#]?\s*(TOP[-\s]?SECRET|SECRET|CONFIDENTIAL|[A-Z]{2,3}\/SCI)\b/gi,
...
export const RECRUITER_REF: PIIPattern = {
  type: "RECRUITER_REF",
  regex:
    /\b(?:RECRUITER|AGENCY)[-\s]?(?:REF|ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
