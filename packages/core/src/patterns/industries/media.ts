...
export const SOURCE_ID: PIIPattern = {
  ...
  validator: (_value: string, context: string) => {
    return /source|informant|confidential|anonymous|whistleblower/i.test(
      context,
    );
  },
};
...
export const MANUSCRIPT_ID: PIIPattern = {
  type: "MANUSCRIPT_ID",
  regex:
    /\b(?:MANUSCRIPT|MS)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const PRESS_PASS_ID: PIIPattern = {
  type: "PRESS_PASS_ID",
  regex:
    /\b(?:PRESS[-\s]?PASS|MEDIA[-\s]?CREDENTIAL)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const CONTRIBUTOR_ID: PIIPattern = {
  type: "CONTRIBUTOR_ID",
  regex:
    /\b(?:CONTRIBUTOR|FREELANCER|WRITER)[-\s]?(?:ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const PUBLISHING_CONTRACT: PIIPattern = {
  ...
  regex:
    /\b(?:PUBLISHING|PUB)[-\s]?(?:CONTRACT)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const EDITORIAL_TICKET: PIIPattern = {
  ...
  regex:
    /\b(?:EDITORIAL|EDIT)[-\s]?(?:TICKET|TASK)?[-\s]?(?:ID|NO)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[EDIT_{n}]",
  priority: 70,
  severity: "low",
...
export const ISBN: PIIPattern = {
  type: "ISBN",
  regex:
    /\bISBN[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{3}[-\s]?\d{1,5}[-\s]?\d{1,7}[-\s]?\d{1,7}[-\s]?\d{1})\b/gi,
  placeholder: "[ISBN_{n}]",
  priority: 65,
  severity: "low",
...
export const COPYRIGHT_REG: PIIPattern = {
  type: "COPYRIGHT_REG",
  regex:
    /\b(?:COPYRIGHT|©)[-\s]?(?:REG(?:ISTRATION)?)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{2,3}\d{6,10})\b/gi,
