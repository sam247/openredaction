...
export const SUPPLIER_ID: PIIPattern = {
  type: "SUPPLIER_ID",
  regex:
    /\bSUPP(?:LIER)?[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{2}\d{5,8})\b/gi,
...
export const PART_NUMBER: PIIPattern = {
  type: "PART_NUMBER",
  regex:
    /\bP(?:ART)?N[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([0-9A-Z]{6,12})\b/gi,
  placeholder: "[PART_{n}]",
  priority: 70,
  severity: "low",
...
export const PURCHASE_ORDER_NUMBER: PIIPattern = {
  ...
  regex:
    /\bP(?:URCHASE[-\s]?)?O(?:RDER)?[-\s]+(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
  placeholder: "[PO_{n}]",
  priority: 85,
  severity: "medium",
...
export const WORK_ORDER_NUMBER: PIIPattern = {
  ...
  regex:
    /\bW(?:ORK[-\s]?)?O(?:RDER)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[WO_{n}]",
  priority: 80,
  severity: "medium",
...
export const BATCH_LOT_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:BATCH|LOT)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const MANUFACTURING_SERIAL: PIIPattern = {
  ...
  regex:
    /\b(?:SERIAL|SN)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
...
export const QC_CERTIFICATE_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:QC|QUALITY)[-\s]?(?:CERT(?:IFICATE)?)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
  placeholder: "[QC_{n}]",
  priority: 80,
  severity: "medium",
...
export const CONTAINER_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:CONTAINER|CNTR)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{4}\d{7})\b/gi,
...
export const PALLET_ID: PIIPattern = {
  type: "PALLET_ID",
  regex:
    /\bPALLET[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
