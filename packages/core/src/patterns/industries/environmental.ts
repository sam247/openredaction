...
export const EPA_ID_NUMBER: PIIPattern = {
  type: "EPA_ID_NUMBER",
  regex:
    /\bEPA[-\s]?(?:ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{2}[A-Z0-9]{9})\b/gi,
...
export const NPDES_PERMIT: PIIPattern = {
  type: "NPDES_PERMIT",
  regex:
    /\bNPDES[-\s]?(?:PERMIT|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{2}[A-Z0-9]{7,9})\b/gi,
...
export const HAZARDOUS_WASTE_MANIFEST: PIIPattern = {
  ...
  regex:
    /\b(?:MANIFEST|WASTE)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*(\d{9})\b/gi,
...
export const AIR_QUALITY_PERMIT: PIIPattern = {
  ...
  regex:
    /\b(?:AIR|EMISSION)[-\s]?PERMIT[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const WATER_QUALITY_CERTIFICATE: PIIPattern = {
  ...
  regex:
    /\bWATER[-\s]?(?:QUALITY|CERT(?:IFICATE)?)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const STORM_WATER_PERMIT: PIIPattern = {
  ...
  regex:
    /\bSTORM\s?WATER[-\s]?PERMIT[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{2}[A-Z0-9]{6,10})\b/gi,
...
export const TRI_FACILITY_ID: PIIPattern = {
  ...
  regex:
    /\bTRI[-\s]?(?:FACILITY|ID)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*(\d{11})\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return /tri|toxic\s?release|inventory|facility|epa|emissions/i.test(
      context,
    );
  },
};
...
export const SPILL_REPORT_NUMBER: PIIPattern = {
  ...
  regex:
    /\bSPILL[-\s]?(?:REPORT|NO|NUM|NUMBER)[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[SPILL_{n}]",
  priority: 80,
  severity: "medium",
  description: "Environmental Spill Report Number",
  validator: (_value: string, context: string) => {
    return /spill|release|incident|environmental|response|cleanup/i.test(
      context,
    );
  },
};
...
export const REMEDIATION_SITE_ID: PIIPattern = {
  ...
  regex:
    /\b(?:REMEDIATION|CLEANUP)[-\s]?SITE[-\s]?(?:ID|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[REMEDIATION_{n}]",
  priority: 80,
  severity: "low",
  description: "Remediation Site ID",
  validator: (_value: string, context: string) => {
    return /remediation|cleanup|site|superfund|brownfield|contamination/i.test(
      context,
    );
  },
};
...
export const ENVIRONMENTAL_CERTIFICATE: PIIPattern = {
  ...
  regex:
    /\bENVIRONMENTAL[-\s]?(?:CERT(?:IFICATE)?|COMPLIANCE)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[ENV_CERT_{n}]",
  priority: 75,
  severity: "low",
  description: "Environmental Compliance Certificate",
  validator: (_value: string, context: string) => {
    return /environmental|compliance|certificate|permit|authorization/i.test(
      context,
    );
  },
};
