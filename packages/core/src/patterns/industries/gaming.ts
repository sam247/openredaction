...
export const RIOT_ID: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation
    return /riot|league[- ]?of[- ]?legends|valorant|tft|teamfight[- ]?tactics|gaming/i.test(
      context,
    );
  },
};
...
export const TWITCH_USERNAME: PIIPattern = {
  ...
  regex:
    /\bTWITCH[-\s]?(?:USER|NAME|ID)?[-\s]?[:#]?\s*([a-zA-Z0-9_]{4,25})\b/gi,
...
export const ESPORTS_PLAYER_ID: PIIPattern = {
  ...
  regex:
    /\b(?:PLAYER|COMPETITOR|PARTICIPANT)[-\s]?(?:ID|NO|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[PLAYER_ID_{n}]",
  priority: 75,
  severity: "medium",
  description: "Esports player/competitor ID",
  validator: (_value: string, context: string) => {
    return /esports|tournament|competition|player|competitor|gaming|league/i.test(
      context,
    );
  },
};
...
export const TOURNAMENT_REGISTRATION_ID: PIIPattern = {
  ...
  regex:
    /\b(?:TOURNAMENT|BRACKET|REGISTRATION|REG)[-\s]?(?:ID|NO|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[TOURNEY_{n}]",
  priority: 75,
  severity: "medium",
  description: "Gaming tournament registration ID",
  validator: (_value: string, context: string) => {
    return /tournament|bracket|registration|competition|event|gaming|esports/i.test(
      context,
    );
  },
};
...
export const FORTNITE_ACCOUNT_ID: PIIPattern = {
  ...
  regex:
    /\b(?:FORTNITE|FN)[-\s]?(?:ACCOUNT|USER|ID)?[-\s]?[:#]?\s*([a-f0-9]{32})\b/gi,
...
export const CSGO_FRIEND_CODE: PIIPattern = {
  ...
  regex:
    /\b(?:CS:?GO|COUNTER[- ]?STRIKE)[-\s]?(?:FRIEND[- ]?CODE|CODE)?[-\s]?[:#]?\s*([A-Z0-9]{5}-[A-Z0-9]{5})\b/gi,
