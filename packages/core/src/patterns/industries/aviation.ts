...
export const IATA_AIRPORT_CODE: PIIPattern = {
  ...
  regex:
    /\b(?:AIRPORT|FROM|TO|VIA|IATA)[-\s]?(?:CODE)?[-\s]?[:#]?\s*([A-Z]{3})\b/gi,
...
export const FLIGHT_NUMBER: PIIPattern = {
  type: "FLIGHT_NUMBER",
  regex:
    /\b(?:FLIGHT|FLT)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{2,3}\s?\d{1,4})\b/gi,
...
export const AIRCRAFT_REGISTRATION: PIIPattern = {
  ...
  regex:
    /\b(?:REGISTRATION|REG|TAIL)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{1,2}-[A-Z0-9]{3,5})\b/gi,
...
export const FAA_AIRMAN_CERTIFICATE: PIIPattern = {
  ...
  regex:
    /\b(?:FAA|AIRMAN|PILOT)[-\s]?(?:CERT(?:IFICATE)?|LICENSE)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*(\d{7,8})\b/gi,
...
export const ICAO_AIRCRAFT_TYPE: PIIPattern = {
  ...
  regex:
    /\b(?:AIRCRAFT|TYPE|ICAO)[-\s]?(?:CODE|DESIGNATOR)?[-\s]?[:#]?\s*([A-Z][0-9][A-Z0-9]{1,2})\b/gi,
...
export const AIRLINE_BOOKING_REFERENCE: PIIPattern = {
  ...
  regex:
    /\b(?:BOOKING|RESERVATION|LOCATOR|REFERENCE)[-\s]?(?:CODE|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6})\b/gi,
  placeholder: "[BOOKING_REF_{n}]",
  priority: 85,
  severity: "medium",
  description: "Airline Booking Reference/Locator",
  validator: (_value: string, context: string) => {
    return /booking|reservation|locator|reference|pnr|airline|flight/i.test(
      context,
    );
  },
};
...
export const IATA_AIRLINE_CODE: PIIPattern = {
  ...
  regex:
    /\b(?:AIRLINE|CARRIER)[-\s]?(?:CODE|IATA)?[-\s]?[:#]?\s*([A-Z]{2})\b/gi,
