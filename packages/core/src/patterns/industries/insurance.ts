...
export const CLAIM_ID: PIIPattern = {
  ...
  validator: (_value: string, context: string) => {
    return /claim|insurance|policy|accident|loss|damage|settlement/i.test(
      context,
    );
  },
};
...
export const POLICY_NUMBER: PIIPattern = {
  type: "POLICY_NUMBER",
  regex:
    /\bPOLICY[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{2,4}\d{6,10})\b/gi,
...
export const POLICY_HOLDER_ID: PIIPattern = {
  ...
  regex:
    /\b(?:POLICY[-\s]?HOLDER|INSURED|POLICYHOLDER)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9-]{8,14})\b/gi,
...
export const QUOTE_REFERENCE: PIIPattern = {
  ...
  regex:
    /\b(?:QUOTE|QTE)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const INSURANCE_CERTIFICATE: PIIPattern = {
  ...
  regex:
    /\b(?:CERTIFICATE|CERT)[-\s]?(?:OF[-\s]?INSURANCE)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
  placeholder: "[CERT_{n}]",
  priority: 80,
  severity: "high",
...
export const ADJUSTER_ID: PIIPattern = {
  type: "ADJUSTER_ID",
  regex:
    /\b(?:ADJUSTER|ADJ)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const UNDERWRITER_ID: PIIPattern = {
  type: "UNDERWRITER_ID",
  regex:
    /\b(?:UNDERWRITER|UW)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[UW_{n}]",
  priority: 75,
  severity: "medium",
...
export const INCIDENT_REPORT_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:INCIDENT|ACCIDENT)[-\s]?(?:REPORT)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const PREMIUM_PAYMENT_REF: PIIPattern = {
  ...
  regex:
    /\b(?:PREMIUM)[-\s]?(?:PAYMENT)?[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const REINSURANCE_TREATY: PIIPattern = {
  ...
  regex:
    /\b(?:REINSURANCE|TREATY)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
