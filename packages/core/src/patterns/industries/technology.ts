...
export const GENERIC_SECRET: PIIPattern = {
  type: "GENERIC_SECRET",
  regex:
    /\b(?:password|passwd|pwd|secret)[:\s=]+([a-zA-Z0-9!@#$%^&*()_+\-=[\]{}|;:,.<>?]{8,})\b/gi,
  ...
  validator: (value: string, _context: string) => {
    ...
    const excluded =
      /example|sample|test|fake|demo|placeholder|xxx|password|secret|\*+/i;
...
export const PRIVATE_KEY: PIIPattern = {
  type: "PRIVATE_KEY",
  regex:
    /-----BEGIN (?:RSA |EC )?PRIVATE KEY-----[\s\S]{20,}?-----END (?:RSA |EC )?PRIVATE KEY-----/g,
...
export const SSH_PRIVATE_KEY: PIIPattern = {
  ...
  regex:
    /-----BEGIN OPENSSH PRIVATE KEY-----[\s\S]{20,}?-----END OPENSSH PRIVATE KEY-----/g,
...
export const AZURE_RESOURCE_ID: PIIPattern = {
  ...
  regex:
    /\/subscriptions\/[a-f0-9-]{36}\/resourceGroups\/[a-zA-Z0-9\-_]+\/providers\/[a-zA-Z0-9.\-_/]+/gi,
...
export const GCP_SERVICE_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\{[^}]*"type"\s*:\s*"service_account"[^}]*"private_key_id"\s*:\s*"([a-z0-9]{40})"[^}]*\}/gi,
...
export const KUBERNETES_SECRET: PIIPattern = {
  ...
  regex:
    /\b(?:kind:\s*Secret|apiVersion:\s*v1)\s[\s\S]{0,500}?data:\s*\n\s+[a-zA-Z0-9\-_]+:\s*([A-Za-z0-9+/=]{20,})/g,
...
export const COOKIE_SESSION: PIIPattern = {
  type: "COOKIE_SESSION",
  regex:
    /\b(?:set-cookie|cookie):\s*(?:session|sessid|sid|auth)=([a-zA-Z0-9_\-.]{20,})/gi,
