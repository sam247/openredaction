...
export const SWIFT_BIC: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const financialContext =
      /swift|bic|bank|transfer|wire|international|payment/i.test(context);
...
export const TRANSACTION_ID: PIIPattern = {
  type: "TRANSACTION_ID",
  regex:
    /\b(?:TXN|TX|TRANS(?:ACTION)?)[-\s]?(?:ID|NO|NUM(?:BER)?|REF)?[-\s]?[:#]?\s*([A-Z0-9]{8,20})\b/gi,
  placeholder: "[TXN_{n}]",
  priority: 80,
  severity: "medium",
...
export const INVESTMENT_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\b(?:ISA|SIPP|INV(?:ESTMENT)?|PENSION|401K|IRA)[-\s\u00A0]*(?:ACCOUNT|ACCT|A\/C)?[-\s\u00A0]*(?:NO|NUM(?:BER)?)?[-\s\u00A0.:#]*([A-Z0-9](?:[A-Z0-9][\s\u00A0./-]?){5,18}[A-Z0-9])\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    const inContext = /isa|sipp|invest|pension|401k|ira|account|fund/i.test(
      context,
    );
...
export const WIRE_TRANSFER_REF: PIIPattern = {
  ...
  regex:
    /\b(?:WIRE|TRANSFER|REMITTANCE)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,20})\b/gi,
  placeholder: "[WIRE_{n}]",
  priority: 85,
  severity: "high",
...
export const DD_MANDATE: PIIPattern = {
  type: "DD_MANDATE",
  regex:
    /\b(?:DD|DIRECT[-\s]?DEBIT)[-\s]?(?:MANDATE|REF(?:ERENCE)?|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,18})\b/gi,
...
export const CHEQUE_NUMBER: PIIPattern = {
  type: "CHEQUE_NUMBER",
  regex:
    /\b(?:CHE(?:QUE|CK))[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{6,10})\b/gi,
...
export const TRADING_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\b(?:TRADING|BROKERAGE|STOCK)[-\s]?(?:ACCOUNT|ACCT|A\/C)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const LOAN_ACCOUNT: PIIPattern = {
  type: "LOAN_ACCOUNT",
  regex:
    /\b(?:LOAN|MORTGAGE|CREDIT)[-\s]?(?:ACCOUNT|ACCT|A\/C)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,16})\b/gi,
  placeholder: "[LOAN_{n}]",
  priority: 85,
  severity: "high",
...
export const CRYPTO_TX_HASH: PIIPattern = {
  ...
  validator: (_value: string, context: string) => {
    return /crypto|bitcoin|ethereum|blockchain|transaction|tx|txid/i.test(
      context,
    );
  },
};
...
export const CARD_EXPIRY_IN_CONTEXT: PIIPattern = {
  type: "CARD_EXPIRY",
  regex:
    /\b(?:EXP(?:IRY|IRATION)?|VALID\s+THRU)[:\s]+(\d{2}[/-]\d{2,4}|\d{4})\b/gi,
  placeholder: "[EXPIRY_{n}]",
  priority: 90,
  severity: "high",
  description: "Card expiration dates",
  validator: (value: string, context: string) => {
    const cardContext = /card|payment|credit|debit|visa|mastercard|amex/i.test(
      context,
    );
...
export const STOCK_TRADE: PIIPattern = {
  type: "STOCK_TRADE",
  regex:
    /\b([A-Z]{1,5})\s+(?:BUY|SELL|SOLD|BOUGHT)\s+(\d+(?:,\d{3})*(?:\.\d{2})?)\s+(?:@|at)\s+\$?(\d+(?:\.\d{2,4})?)\b/gi,
...
export const WIRE_TRANSFER_DETAILS: PIIPattern = {
  ...
  regex:
    /\b(?:WIRE\s+TO|TRANSFER\s+TO|BENEFICIARY)[:\s]+([A-Z0-9\s,.-]{20,100})/gi,
  placeholder: "[WIRE_DETAILS_{n}]",
  priority: 90,
  severity: "high",
  description: "Bank wire transfer beneficiary details",
  validator: (_value: string, context: string) => {
    return /wire|transfer|beneficiary|recipient|iban|swift|aba|routing/i.test(
      context,
    );
  },
};
...
export const STATEMENT_REF: PIIPattern = {
  type: "STATEMENT_REF",
  regex:
    /\b(?:STATEMENT|STMT)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[STMT_{n}]",
  priority: 75,
  severity: "medium",
...
export const STANDING_ORDER_REF: PIIPattern = {
  ...
  regex:
    /\b(?:STANDING[-\s]?ORDER|SO)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[SO_{n}]",
  priority: 75,
  severity: "medium",
...
export const PAYMENT_REFERENCE: PIIPattern = {
  ...
  regex:
    /\b(?:PAYMENT|PAY)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,20})\b/gi,
...
export const CARD_AUTH_CODE: PIIPattern = {
  type: "CARD_AUTH_CODE",
  regex:
    /\b(?:AUTH(?:ORIZATION)?|APPROVAL)[-\s]?(?:CODE|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[AUTH_{n}]",
  priority: 80,
  severity: "high",
...
export const MERCHANT_ID: PIIPattern = {
  type: "MERCHANT_ID",
  regex:
    /\b(?:MERCHANT|MID)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,20})\b/gi,
...
export const TERMINAL_ID: PIIPattern = {
  type: "TERMINAL_ID",
  regex:
    /\b(?:TERMINAL|TID|POS)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,16})\b/gi,
...
export const UK_BANK_ACCOUNT_IBAN: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const bankingKeywords =
      /iban|account|bank|uk|gb|financial|payment|transfer/i;
    ...
    const rejectKeywords =
      /example\s+iban|test\s+iban|sample\s+iban|demo\s+iban|fake\s+iban/i;
...
export const UK_SORT_CODE_ACCOUNT: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Must have banking context
    const bankingKeywords =
      /sort\s+code|account|bank|uk|gb|financial|payment|transfer/i;
    if (!bankingKeywords.test(context)) {
      return false;
    }
    ...
    // Reject test/example keywords
    const rejectKeywords =
      /example\s+account|test\s+account|sample\s+account|demo\s+account|fake\s+account/i;
    if (rejectKeywords.test(context)) {
      return false;
    }
