...
export const AIRLINE_PNR: PIIPattern = {
  type: "AIRLINE_PNR",
  regex:
    /\b(?:PNR|BOOKING|CONFIRMATION)[-\s]?(?:NO|NUM|NUMBER|CODE)?[-\s]?[:#]?\s*([A-Z0-9]{6})\b/gi,
  placeholder: "[PNR_{n}]",
  priority: 85,
  severity: "medium",
  ...
  validator: (value: string, context: string) => {
    ...
    return /airline|flight|booking|reservation|pnr|travel|passenger|ticket/i.test(
      context,
    );
  },
};
...
export const HOTEL_RESERVATION: PIIPattern = {
  ...
  regex:
    /\b(?:HOTEL|RESERVATION|CONF(?:IRMATION)?|BOOKING)[-\s]?(?:NO|NUM|NUMBER|CODE)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
  placeholder: "[HOTEL_RES_{n}]",
  priority: 80,
  severity: "medium",
  description: "Hotel reservation/confirmation number",
  validator: (_value: string, context: string) => {
    return /hotel|reservation|booking|room|accommodation|stay|check[-\s]?in|lodging/i.test(
      context,
    );
  },
};
...
export const FREQUENT_FLYER_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:FREQUENT[- ]?FLYER|FF|MILEAGE|LOYALTY)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[FF_NUM_{n}]",
  priority: 75,
  severity: "medium",
  description: "Frequent flyer/loyalty program number",
  validator: (_value: string, context: string) => {
    return /frequent[- ]?flyer|miles|mileage|loyalty|rewards|member|airline/i.test(
      context,
    );
  },
};
...
export const HOTEL_LOYALTY_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:MEMBER|LOYALTY|REWARDS)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[HOTEL_LOYALTY_{n}]",
  priority: 75,
  severity: "medium",
  description: "Hotel loyalty/rewards program number",
  validator: (_value: string, context: string) => {
    return /hotel|marriott|hilton|hyatt|ihg|loyalty|rewards|points|member/i.test(
      context,
    );
  },
};
...
export const CRUISE_BOOKING_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:CRUISE|BOOKING|RESERVATION)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[CRUISE_{n}]",
  priority: 75,
  severity: "medium",
  description: "Cruise line booking number",
  validator: (_value: string, context: string) => {
    return /cruise|ship|sailing|voyage|cabin|carnival|royal[- ]?caribbean|norwegian/i.test(
      context,
    );
  },
};
...
export const TRAVEL_AGENCY_BOOKING: PIIPattern = {
  ...
  regex:
    /\b(?:TRAVEL|AGENCY|BOOKING|TRIP)[-\s]?(?:REF|REFERENCE|NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[TRAVEL_REF_{n}]",
  priority: 70,
  severity: "medium",
  description: "Travel agency booking reference",
  validator: (_value: string, context: string) => {
    return /travel[- ]?agency|tour|package|itinerary|booking|vacation/i.test(
      context,
    );
  },
};
...
export const RENTAL_CAR_CONFIRMATION: PIIPattern = {
  ...
  regex:
    /\b(?:RENTAL|CAR|VEHICLE)[-\s]?(?:CONF(?:IRMATION)?|RESERVATION|BOOKING)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[CAR_RENTAL_{n}]",
  priority: 75,
  severity: "medium",
  description: "Rental car confirmation number",
  validator: (_value: string, context: string) => {
    return /rental|car|vehicle|hertz|enterprise|avis|budget|rent/i.test(
      context,
    );
  },
};
...
export const THEME_PARK_TICKET: PIIPattern = {
  ...
  regex:
    /\b(?:TICKET|PASS|ADMISSION)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return /theme[- ]?park|disney|universal|attraction|admission|ticket|pass|entry/i.test(
      context,
    );
  },
};
...
export const TSA_PRECHECK_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:TSA|PRECHECK|KTN|KNOWN[- ]?TRAVELER)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z0-9]{9,10})\b/gi,
  placeholder: "[TSA_{n}]",
  priority: 85,
  severity: "medium",
  ...
  validator: (value: string, context: string) => {
    ...
    return /tsa|precheck|pre[- ]?check|ktn|known[- ]?traveler|security|screening/i.test(
      context,
    );
  },
};
...
export const GLOBAL_ENTRY_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:GLOBAL[- ]?ENTRY|PASS[- ]?ID)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*(\d{9})\b/gi,
  ...
  validator: (value: string, context: string) => {
    ...
    return /global[- ]?entry|pass[- ]?id|customs|immigration|cbp|trusted[- ]?traveler/i.test(
      context,
    );
  },
};
