...
export const SIM_CARD_NUMBER: PIIPattern = {
  ...
  regex:
    /\bSIM[-\s]?(?:CARD)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{19,20})\b/gi,
  placeholder: "[SIM_{n}]",
  priority: 85,
  severity: "high",
...
export const SERVICE_REQUEST_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:SERVICE|SR)[-\s]?(?:REQUEST)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const UTILITY_BILL_ACCOUNT: PIIPattern = {
  ...
  regex:
    /\b(?:BILL|BILLING)[-\s]?(?:ACCOUNT)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*(\d{8,14})\b/gi,
  placeholder: "[BILL_{n}]",
  priority: 85,
  severity: "high",
...
export const INSTALLATION_REF: PIIPattern = {
  ...
  regex:
    /\b(?:INSTALLATION|INSTALL)[-\s]?(?:REF(?:ERENCE)?|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const PHONE_LINE_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:LINE|NUMBER)[-\s]?(?:NO)?[-\s]?[:#]?\s*(\d{3}[-\s]?\d{3}[-\s]?\d{4})\b/g,
...
export const BROADBAND_SERVICE_ID: PIIPattern = {
  ...
  regex:
    /\b(?:BROADBAND|INTERNET|ISP)[-\s]?(?:SERVICE)?[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
...
export const EQUIPMENT_SERIAL: PIIPattern = {
  ...
  regex:
    /\b(?:EQUIPMENT|DEVICE|ROUTER|MODEM)[-\s]?(?:SERIAL)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{10,16})\b/gi,
...
export const SMART_METER_ID: PIIPattern = {
  type: "SMART_METER_ID",
  regex:
    /\b(?:SMART[-\s]?METER)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{10,16})\b/gi,
