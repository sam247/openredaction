...
export const CASE_NUMBER: PIIPattern = {
  type: "CASE_NUMBER",
  regex:
    /\b(?:CASE|DOCKET|FILE)[-\s]?(?:NO|NUM(?:BER)?|REF)?[-\s]?[:#]?\s*([A-Z]{1,3}[-]?\d{2,4}[-]?[A-Z]{0,3}[-]?\d{4,8})\b/gi,
  placeholder: "[CASE_{n}]",
  priority: 85,
  severity: "high",
...
export const MATTER_NUMBER: PIIPattern = {
  type: "MATTER_NUMBER",
  regex:
    /\b(?:MATTER|ENGAGEMENT|CLIENT[-\s]?MATTER)[-\s]?(?:NO|NUM(?:BER)?|REF|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
  placeholder: "[MATTER_{n}]",
  priority: 80,
  severity: "high",
  description: "Law firm matter and engagement numbers",
  validator: (_value: string, context: string) => {
    return /legal|matter|client|engagement|firm|attorney|counsel/i.test(
      context,
    );
  },
};
...
export const BAR_NUMBER: PIIPattern = {
  type: "BAR_NUMBER",
  regex:
    /\b(?:BAR|ATTORNEY|LAWYER)[-\s]?(?:NO|NUM(?:BER)?|REG(?:ISTRATION)?|LIC(?:ENSE)?)?[-\s]?[:#]?\s*([A-Z0-9]{5,12})\b/gi,
  placeholder: "[BAR_{n}]",
  priority: 85,
  severity: "high",
...
export const DEPOSITION_REF: PIIPattern = {
  type: "DEPOSITION_REF",
  regex:
    /\b(?:DEPOSITION|DEPO|DEP)[-\s]?(?:NO|NUM(?:BER)?|REF|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[DEPO_{n}]",
  priority: 75,
  severity: "medium",
...
export const DISCOVERY_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:DISCOVERY|INTERROGATORY|REQUEST|RFP|RFA)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{1,4}[-]?\d{1,4})\b/gi,
...
export const COURT_REPORTER_LICENSE: PIIPattern = {
  ...
  regex:
    /\b(?:COURT[-\s]?REPORTER|CSR|RPR)[-\s]?(?:LIC(?:ENSE)?|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{2,3}[-]?\d{4,8})\b/gi,
  placeholder: "[CSR_{n}]",
  priority: 75,
  severity: "medium",
...
export const SUBPOENA_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:SUBPOENA|SUMMONS)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,15})\b/gi,
...
export const JUDGMENT_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:JUDGMENT|ORDER|DECREE)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const PATENT_NUMBER: PIIPattern = {
  type: "PATENT_NUMBER",
  regex:
    /\b(?:(?:US|EP|WO|PCT)[-\s]?)?(?:PATENT|PAT)[-\s]?(?:NO|NUM(?:BER)?|APPL(?:ICATION)?)?[-\s]?[:#]?\s*([A-Z]{0,2}\d{6,10}[A-Z0-9]{0,3})\b/gi,
...
export const SETTLEMENT_ID: PIIPattern = {
  type: "SETTLEMENT_ID",
  regex:
    /\b(?:SETTLEMENT|AGREEMENT)[-\s]?(?:ID|NO|NUM(?:BER)?|REF)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[SETTLEMENT_{n}]",
  priority: 85,
  severity: "high",
  description: "Settlement agreement identifiers",
  validator: (_value: string, context: string) => {
    return /settlement|agreement|resolution|mediation|arbitration/i.test(
      context,
    );
  },
};
...
export const CLIENT_ID: PIIPattern = {
  type: "CLIENT_ID",
  regex:
    /\b(?:CLIENT)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const RETAINER_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:RETAINER)[-\s]?(?:NO|NUM(?:BER)?|AGREEMENT)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const NOTARY_LICENSE: PIIPattern = {
  type: "NOTARY_LICENSE",
  regex:
    /\b(?:NOTARY|NOTARIAL)[-\s]?(?:LIC(?:ENSE)?|COMMISSION|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const BANKRUPTCY_CASE: PIIPattern = {
  ...
  regex:
    /\b(?:BK|BANKRUPTCY)[-\s]?(?:NO|NUM(?:BER)?|CASE)?[-\s]?[:#]?\s*(\d{2}[-]?\d{5}[-]?[A-Z]{0,3})\b/gi,
  placeholder: "[BK_{n}]",
  priority: 85,
  severity: "high",
...
export const PROBATE_CASE: PIIPattern = {
  type: "PROBATE_CASE",
  regex:
    /\b(?:PROBATE|ESTATE)[-\s]?(?:NO|NUM(?:BER)?|CASE)?[-\s]?[:#]?\s*([A-Z]{1,2}\d{6,10})\b/gi,
...
export const NDA_ID: PIIPattern = {
  type: "NDA_ID",
  regex:
    /\b(?:NDA|CONFIDENTIALITY|NON[-\s]?DISCLOSURE)[-\s]?(?:AGREEMENT)?[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[NDA_{n}]",
  priority: 75,
  severity: "high",
