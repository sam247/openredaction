...
export const STUDENT_ID: PIIPattern = {
  type: "STUDENT_ID",
  regex:
    /\b(?:STUDENT|PUPIL|SCHOLAR)[-\s]?(?:ID|NUM(?:BER)?|NO)?[-\s]?[:#]?\s*([A-Z]{0,2}\d{6,10})\b/gi,
...
export const UNIVERSITY_ID: PIIPattern = {
  type: "UNIVERSITY_ID",
  regex:
    /\b(?:UNIVERSITY|COLLEGE|UNI)[-\s]?(?:ID|NUM(?:BER)?|NO)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[UNI_ID_{n}]",
  priority: 90,
  severity: "high",
  description: "University and college ID numbers",
  validator: (_value: string, context: string) => {
    return /university|college|campus|student|enrollment|registr/i.test(
      context,
    );
  },
};
...
export const COURSE_CODE: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    const academicContext =
      /course|class|section|semester|quarter|curriculum|syllabus/i.test(
        context,
      );
...
export const GRADE_REFERENCE: PIIPattern = {
  ...
  regex:
    /\b(?:GPA|GRADE[-\s]?POINT[-\s]?AVERAGE)[-\s]?[:#]?\s*((?:[0-4]\.\d{1,2})|(?:\d\.\d{2}))\b/gi,
  placeholder: "[GPA_{n}]",
  priority: 80,
  severity: "high",
...
export const TRANSCRIPT_ID: PIIPattern = {
  type: "TRANSCRIPT_ID",
  regex:
    /\b(?:TRANSCRIPT)[-\s]?(?:ID|NUM(?:BER)?|NO|REF)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const LIBRARY_CARD: PIIPattern = {
  type: "LIBRARY_CARD",
  regex:
    /\b(?:LIBRARY)[-\s]?(?:CARD|ID|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const FACULTY_ID: PIIPattern = {
  type: "FACULTY_ID",
  regex:
    /\b(?:FACULTY|TEACHER|INSTRUCTOR|PROFESSOR|STAFF)[-\s]?(?:ID|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{1,2}\d{6,10})\b/gi,
  placeholder: "[FACULTY_{n}]",
  priority: 85,
  severity: "high",
  description: "Faculty and teacher identification numbers",
  validator: (_value: string, context: string) => {
    return /faculty|teacher|instructor|professor|staff|employee|personnel/i.test(
      context,
    );
  },
};
...
export const ENROLLMENT_NUMBER: PIIPattern = {
  ...
  regex:
    /\b(?:ENROLLMENT|REGISTRATION)[-\s]?(?:NO|NUM(?:BER)?|ID)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const FINANCIAL_AID_ID: PIIPattern = {
  ...
  regex:
    /\b(?:FINANCIAL[-\s]?AID|FAFSA|AID[-\s]?APPLICATION)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
  placeholder: "[AID_{n}]",
  priority: 90,
  severity: "high",
...
export const DEGREE_NUMBER: PIIPattern = {
  type: "DEGREE_NUMBER",
  regex:
    /\b(?:DEGREE|DIPLOMA|CERTIFICATE)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
...
export const EXAM_ID: PIIPattern = {
  type: "EXAM_ID",
  regex:
    /\b(?:EXAM|TEST|QUIZ|ASSESSMENT)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[EXAM_{n}]",
  priority: 75,
  severity: "medium",
...
export const HOUSING_ASSIGNMENT: PIIPattern = {
  ...
  regex:
    /\b(?:DORM|ROOM|HOUSING)[-\s]?(?:NO|NUM(?:BER)?|ASSIGNMENT)?[-\s]?[:#]?\s*([A-Z0-9]{4,10})\b/gi,
...
export const MEAL_PLAN_ID: PIIPattern = {
  type: "MEAL_PLAN_ID",
  regex:
    /\b(?:MEAL[-\s]?PLAN|DINING)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[MEAL_{n}]",
  priority: 70,
  severity: "medium",
...
export const PARKING_PERMIT: PIIPattern = {
  type: "PARKING_PERMIT",
  regex:
    /\b(?:PARKING)[-\s]?(?:PERMIT|PASS|DECAL)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const SCHOLARSHIP_ID: PIIPattern = {
  type: "SCHOLARSHIP_ID",
  regex:
    /\b(?:SCHOLARSHIP|GRANT|AWARD)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
  placeholder: "[SCHOLARSHIP_{n}]",
  priority: 80,
  severity: "high",
  description: "Scholarship and grant award numbers",
  validator: (_value: string, context: string) => {
    return /scholarship|grant|award|fellowship|stipend|financial/i.test(
      context,
    );
  },
};
...
export const GRADUATION_YEAR: PIIPattern = {
  ...
  regex:
    /\b(?:CLASS[-\s]?OF|GRADUATING[-\s]?CLASS|GRAD(?:UATION)?[-\s]?YEAR)[-\s]?[:#]?\s*(['']?\d{2}|[12]\d{3})\b/gi,
...
export const APPLICATION_ID: PIIPattern = {
  type: "APPLICATION_ID",
  regex:
    /\b(?:APPLICATION|ADMISSION|APPLICANT)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,14})\b/gi,
  placeholder: "[APPLICATION_{n}]",
  priority: 85,
  severity: "high",
  description: "Admissions application identifiers",
  validator: (_value: string, context: string) => {
    return /application|admission|applicant|prospective|admit|enroll/i.test(
      context,
    );
  },
};
...
export const ALUMNI_ID: PIIPattern = {
  type: "ALUMNI_ID",
  regex:
    /\b(?:ALUMNI|ALUMNUS|ALUMNA)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const RESEARCH_GRANT: PIIPattern = {
  type: "RESEARCH_GRANT",
  regex:
    /\b(?:GRANT|RESEARCH|FUNDING)[-\s]?(?:NO|NUM(?:BER)?|ID|REF)?[-\s]?[:#]?\s*([A-Z]{2,4}[-]?\d{6,10})\b/gi,
...
export const DEPARTMENT_CODE: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    const academicContext = /department|college|school|faculty|division/i.test(
      context,
    );
