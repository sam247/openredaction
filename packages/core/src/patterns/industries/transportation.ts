...
export const LICENSE_PLATE: PIIPattern = {
  type: "LICENSE_PLATE",
  regex:
    /\b(?:LICENSE|PLATE|REG(?:ISTRATION)?)[-\s]?(?:NO|NUM(?:BER)?|PLATE)?[-\s]?[:#]?\s*([A-Z0-9]{2,8})\b/gi,
...
export const FLEET_VEHICLE_ID: PIIPattern = {
  ...
  regex:
    /\b(?:FLEET|VEHICLE)[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const TELEMATICS_DEVICE_ID: PIIPattern = {
  ...
  regex:
    /\b(?:TELEMATICS|GPS[-\s]?DEVICE)[-\s]?(?:ID)?[-\s]?[:#]?\s*([A-Z0-9]{10,16})\b/gi,
...
export const BOOKING_NUMBER: PIIPattern = {
  type: "BOOKING_NUMBER",
  regex:
    /\b(?:BOOKING|RESERVATION|RES)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const DRIVER_ID: PIIPattern = {
  type: "DRIVER_ID",
  regex:
    /\bDRIVER[-\s]?(?:ID|NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{6,12})\b/gi,
...
export const SHIPMENT_TRACKING: PIIPattern = {
  ...
  regex:
    /\b(?:SHIPMENT|TRACKING)[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{10,30})\b/gi,
...
export const TOLL_TAG_ID: PIIPattern = {
  type: "TOLL_TAG_ID",
  regex:
    /\b(?:TOLL[-\s]?TAG|E[-]?ZPASS|TRANSPONDER)[-\s]?(?:ID)?[-\s]?[:#]?\s*([A-Z0-9]{8,16})\b/gi,
  placeholder: "[TOLL_{n}]",
  priority: 80,
  severity: "medium",
...
export const INSPECTION_CERTIFICATE: PIIPattern = {
  ...
  regex:
    /\b(?:INSPECTION|INSP)[-\s]?(?:CERT(?:IFICATE)?)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z0-9]{8,14})\b/gi,
...
export const VEHICLE_INSURANCE_POLICY: PIIPattern = {
  ...
  regex:
    /\b(?:AUTO|VEHICLE|CAR)[-\s]?(?:INSURANCE)?[-\s]?(?:POLICY)?[-\s]?(?:NO|NUM(?:BER)?)?[-\s]?[:#]?\s*([A-Z]{2,4}\d{6,10})\b/gi,
