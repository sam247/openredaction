...
export const INDONESIA_NIK: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    return /indonesia|indonesian|nik|nomor[- ]?induk|ktp|national[- ]?id/i.test(
      context,
    );
  },
};
...
export const MALAYSIA_MYKAD: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation required
    if (
      !/malaysia|malaysian|mykad|ic[- ]?number|kad[- ]?pengenalan/i.test(
        context,
      )
    ) {
      return false;
...
export const PHILIPPINES_UMID: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation required
    return /philippines|filipino|umid|unified|multipurpose|national[- ]?id/i.test(
      context,
    );
  },
};
...
export const VIETNAM_CCCD: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation required
    return /vietnam|vietnamese|cccd|citizen[- ]?identity|cmnd|national[- ]?id/i.test(
      context,
    );
  },
};
...
export const MYANMAR_NRC: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation required
    return /myanmar|burmese|nrc|national[- ]?registration|identity/i.test(
      context,
    );
  },
};
