...
export const SOUTH_AFRICA_ID: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    if (
      !/south[- ]?africa|rsa|za|national[- ]?id|identity|id[- ]?number/i.test(
        context,
      )
    ) {
      return false;
...
export const GHANA_CARD: PIIPattern = {
  ...
  validator: (value: string, context: string) => {
    ...
    // Context validation required
    return /ghana|ghanaian|ghana[- ]?card|national[- ]?id|identity/i.test(
      context,
    );
  },
};
...
export const MOROCCO_NATIONAL_ID: PIIPattern = {
  ...
  regex:
    /\b(?:CNIE|ID)[-\s]?(?:NO|NUM|NUMBER)?[-\s]?[:#]?\s*([A-Z]{1,2}\d{6,8}|\d{8})\b/gi,
