...
export const financialPatterns: PIIPattern[] = [
  {
    type: "CREDIT_CARD",
    regex:
      /(?<!\d)(?:(?:\d{4}[\s\u00A0.-]?){3}\d{4}|\d{4}[\s\u00A0.-]?\d{6}[\s\u00A0.-]?\d{5})(?!\d)/g,
    priority: 100,
    validator: (match: string, context: string) => {
      ...
      const rejectKeywords =
        /example\s+card|test\s+card|sample\s+card|demo\s+card|fake\s+card/i;
      const allowTestValues = /4532-1234-5678-9010|4532123456789010/i.test(
        match,
      );
  ...
  },
  {
    ...
    regex:
      /\b(?:account|acc|a\/c)[:\s#-]*((?:\d{4}[\s\u00A0-]?\d{4})|(?:\d{2}[\s\u00A0-]?\d{2}[\s\u00A0-]?\d{4}))\b/gi,
    priority: 90,
    ...
    validator: (value: string, context: string) => {
      ...
      const bankingKeywords =
        /account|bank|sort\s+code|financial|payment|transfer|deposit|withdrawal/i;
  ...
  },
  {
    type: "SORT_CODE_UK",
    regex:
      /\b(?:sort[\s\u00A0-]*code|SC)[:\s\u00A0.-]*((?:\d{2}[\s\u00A0.-]?){2}\d{2})\b/gi,
    priority: 90,
    validator: (match: string, context: string) => {
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+sort|test\s+sort|sample\s+sort|demo\s+sort|fake\s+sort/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    regex:
      /\b(?:routing|RTN|ABA)[-\s\u00A0]*(?:number|no|num)?[-\s\u00A0.:#]*((?:\d[\s\u00A0.-]?){9})\b/gi,
    priority: 90,
    validator: (match: string, context: string) => {
      // Normalize separators before validation
      const cleaned = match.replace(/[\s\u00A0.-]/g, "");
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+routing|test\s+routing|sample\s+routing|demo\s+routing|fake\s+routing/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    validator: (value: string, context: string) => {
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+ifsc|test\s+ifsc|sample\s+ifsc|demo\s+ifsc|fake\s+ifsc/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    validator: (match: string, context: string) => {
      ...
      // Must have banking context
      const bankingKeywords =
        /bsb|bank|australia|au|financial|payment|transfer/i;
      if (!bankingKeywords.test(context)) {
        return false;
      }
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+bsb|test\s+bsb|sample\s+bsb|demo\s+bsb|fake\s+bsb/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
