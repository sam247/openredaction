...
export const governmentPatterns: PIIPattern[] = [
  {
    type: "SSN",
    regex:
      /\b(?:SSN|social\s+security)\b[:\s\u00A0#-]*([0-9]{3}[\s\u00A0.-]?[0-9]{2}[\s\u00A0.-]?[0-9]{4})\b/gi,
    priority: 100,
    validator: (match: string, context: string) => {
      ...
      const usContext =
        /ssn|social\s+security|us\b|usa|american|government|tax|irs|federal/i;
      const isTestMode =
        context.includes("SSN:") || context.includes("123-45-6789");
      ...
      const rejectKeywords =
        /example\s+ssn|test\s+ssn|sample\s+ssn|demo\s+ssn|fake\s+ssn/i;
  ...
  },
  {
    type: "PASSPORT_UK",
    regex:
      /\b(?:passport|pass)[:\s\u00A0#-]*((?:\d{3}[\s\u00A0.-]?){2}\d{3})\b/gi,
    priority: 95,
    validator: (match: string, context: string) => {
      // Normalize separators
      const cleaned = match.replace(/[\s\u00A0.-]/g, "");
      ...
      const ukContext =
        /passport|uk\b|british|gb|government|border|travel|immigration/i;
      ...
      const rejectKeywords =
        /example\s+passport|test\s+passport|sample\s+passport|demo\s+passport|fake\s+passport/i;
  ...
  },
  {
    type: "PASSPORT_US",
    regex:
      /\b(?:passport|pass)[:\s\u00A0#-]*(([A-Z0-9][\s\u00A0.-]?){5,8}[A-Z0-9])\b/gi,
    priority: 95,
    placeholder: "[PASSPORT_{n}]",
    ...
    validator: (value: string, context: string) => {
      ...
      const usContext =
        /passport|us\b|usa|american|government|state\s+department|border|travel|immigration/i;
  ...
  },
  {
    ...
    regex:
      /\b(?:NI\b|NINO|national\s+insurance)[:\s\u00A0#-]*([A-CEGHJ-PR-TW-Z]{2}(?:[\s\u00A0.-]?\d{2}){3}[\s\u00A0.-]?[A-D])\b/gi,
    priority: 100,
    validator: (match: string, context: string) => {
      // Normalize separators
      ...
      const ukContext =
        /national\s+insurance|nino|ni\b|uk\b|british|gb|government|tax|benefits|hmrc/i;
      if (!ukContext.test(context)) {
        return false;
      }
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+nino|test\s+nino|sample\s+nino|demo\s+nino|fake\s+nino/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    type: "NHS_NUMBER",
    regex:
      /\b(?:NHS|nhs number)[:\s\u00A0#-]*((?:\d{3}[\s\u00A0.-]?){2}\d{4})\b/gi,
    priority: 95,
    validator: (match: string, context: string) => {
      // Normalize separators
      const cleaned = match.replace(/[\s\u00A0.-]/g, "");
      ...
      const nhsContext =
        /nhs|health|medical|hospital|gp|doctor|patient|clinical/i;
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+nhs|test\s+nhs|sample\s+nhs|demo\s+nhs|fake\s+nhs/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    regex:
      /\b(?:DL|DRIVING|DRIVER(?:'S)?|LICEN[SC]E)?[\s\u00A0#:-]*(?:NO|NUM(?:BER)?|ID)?[\s\u00A0#:-]*([A-Z]{5}[\s\u00A0.-]?\d{2}[\s\u00A0.-]?\d{2}[\s\u00A0.-]?\d{2}[\s\u00A0.-]?[A-Z]{2}[\s\u00A0.-]?\d[\s\u00A0.-]?[A-Z]{2})\b/gi,
    priority: 90,
    ...
    validator: (value: string, context: string) => {
      ...
      const ukContext =
        /driving|license|dl\b|uk\b|british|gb|dvla|vehicle|car/i;
      if (!ukContext.test(context)) {
        return false;
      }
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+license|test\s+license|sample\s+license|demo\s+license|fake\s+license/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    regex:
      /\b(?:DL|driver(?:'s)?\slicense)[:\s\u00A0#-]*([A-Z0-9](?:[A-Z0-9][\s\u00A0.-]?){3,18}[A-Z0-9])\b/gi,
    priority: 90,
    placeholder: "[DRIVING_LICENSE_{n}]",
    ...
    validator: (value: string, context: string) => {
      ...
      const usContext =
        /driving|license|dl\b|us\b|usa|american|dmv|vehicle|car/i;
      if (!usContext.test(context)) {
        return false;
      }
  ...
  },
  {
    type: "TAX_ID",
    regex:
      /\b(?:TIN|tax id|EIN)[:\s\u00A0#-]*(\d{2}(?:[\s\u00A0.-]?\d){7})\b/gi,
    priority: 95,
    ...
    validator: (value: string, context: string) => {
      ...
      if (
        firstTwo === 0 ||
        (firstTwo >= 7 && firstTwo <= 8) ||
        (firstTwo >= 90 && firstTwo <= 99)
      ) {
        return false;
      }
      ...
      // Reject test/example keywords
      const rejectKeywords =
        /example\s+tax|test\s+tax|sample\s+tax|demo\s+tax|fake\s+tax|12-3456789/i;
      if (rejectKeywords.test(context)) {
        return false;
      }
  ...
  },
  {
    ...
    regex:
      /\b(?:TRAVEL\s+DOC(?:UMENT)?|TD)[:\s#-]*([A-Z0-9](?:[A-Z0-9][\s\u00A0.-]?){4,13}[A-Z0-9])\b/gi,
    priority: 92,
  ...
  },
  {
    type: "VISA_NUMBER",
    regex:
      /\b(?:VISA)[:\s#-]*([A-Z0-9](?:[A-Z0-9][\s\u00A0.-]?){6,10}[A-Z0-9])\b/gi,
    priority: 92,
    placeholder: "[VISA_{n}]",
  ...
  },
  {
    ...
    regex:
      /\b(?:IMMIGRATION|ALIEN|A-NUMBER|A#)[:\s#-]*([A-Z]?(?:\d[\s\u00A0.-]?){7,9})\b/gi,
    priority: 92,
  ...
  },
  {
    ...
    regex:
      /\b(?:BCC|BORDER\s+CROSSING)[:\s#-]*([A-Z0-9](?:[A-Z0-9\s\u00A0.-]?){8,13}[A-Z0-9])\b/gi,
    priority: 90,
    placeholder: "[BCC_{n}]",
  ...
  },
  {
    type: "UTR_UK",
    regex:
      /\b(?:UTR|unique taxpayer reference)[:\s#-]*((?:\d[\s\u00A0.-]?){10})\b/gi,
    priority: 95,
    validator: (match) => {
  ...
  },
  {
    type: "VAT_NUMBER",
    regex:
      /\b(?:VAT|vat number)[:\s#-]*([A-Z]{2}(?:[\s\u00A0.-]?[A-Z0-9]){7,12})\b/gi,
    priority: 90,
    validator: (match) => {
  ...
  },
  {
    ...
    regex:
      /\b(?:company number|reg(?:\.|istration)?\s+no(?:\.)?)[:\s#]*([A-Z]{2}\d{6}|\d{8})\b/gi,
    priority: 85,
    validator: (match) => {
  ...
  },
  {
    type: "ITIN",
    regex:
      /\b(?:ITIN|individual taxpayer)[:\s#]*(9\d{2}[-\s]?[7-8]\d[-\s]?\d{4})\b/gi,
    priority: 100,
    validator: (match) => {
