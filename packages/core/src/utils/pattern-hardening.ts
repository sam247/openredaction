...
function validateMod11Checksum(value: string, weights?: number[]): boolean {
  ...
  const defaultWeights = Array.from(
...
function validateMod10Checksum(value: string, weights?: number[]): boolean {
  ...
  const digits = value.split("").map(Number);
  const checkDigit = digits.pop()!;
  const defaultWeights = Array.from({ length: digits.length }, (_, i) =>
    i % 2 === 0 ? 2 : 1,
  );
  const w = weights || defaultWeights;
 ...
 * });
 */
export function enhanceValidator<
  T extends (value: string, context?: string) => boolean,
>(
  baseValidator: T,
  enhancements?: {
  ...
  return ((value: string, context: string = "") => {
    ...
    // Length checks
    if (
      enhancements?.minLength &&
      normalizedValue.length < enhancements.minLength
    ) {
      return false;
    }
    if (
      enhancements?.maxLength &&
      normalizedValue.length > enhancements.maxLength
    ) {
      return false;
    }
    ...
    if (
      enhancements?.contextKeywords &&
      enhancements.contextKeywords.length > 0
    ) {
 ...
 */
export function withSeparatorTolerance<
  T extends (value: string, context?: string) => boolean,
>(validator: T): T {
