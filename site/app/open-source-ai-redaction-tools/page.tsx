...
const title = 'Open Source AI Redaction Tools (Self-Hosted Alternatives for PII)';
const description =
...
const tools = [
  {
    ...
    scenario: 'Use when you want deterministic redaction in Node.js apps before OpenAI or other AI APIs.',
  },
  {
    ...
    scenario: 'Use when your stack is Python and you want NLP-based detection with local control.',
  },
  {
    name: 'AWS Comprehend',
    stack: 'Cloud',
    ...
    scenario: 'Use when managed infrastructure is fine and external processing is acceptable.',
  },
  {
    name: 'Google DLP',
    stack: 'Cloud',
    ...
    scenario: 'Use when you want a managed service with stronger coverage and do not need local execution.',
  },
] as const;
      ...
      <main className="pt-[116px] pb-20">
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 sm:pt-16">
          <div className="max-w-3xl">
            <p className="text-sm uppercase tracking-[0.2em] text-gray-500">Comparison page</p>
            ...
            <p className="mt-5 text-lg text-gray-300 max-w-2xl">
              AI APIs can expose sensitive data if PII is still present in prompts, logs, or exports. Teams usually
              compare local tools, Python libraries, and managed cloud services before choosing a redaction path.
              OpenRedaction is our <Link href="/" className="underline underline-offset-4 hover:text-white">open source redaction</Link> option for Node.js, and you can also read the <Link href="/pii-redaction" className="underline underline-offset-4 hover:text-white">PII redaction</Link> guide for a shorter overview.
            </p>
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-14">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {tools.map((tool) => (
              <article key={tool.name} className="rounded-xl border border-gray-800 bg-gray-950 p-5 sm:p-6">
                ...
                <div className="mt-3 space-y-2 text-sm text-gray-400">
                  ...
                  <p>
                    <span className="text-gray-200">Approach:</span> {tool.approach}
                  </p>
                  <p>
                    <span className="text-gray-200">Strength:</span> {tool.strength}
                  </p>
                  <p>
                    <span className="text-gray-200">Limitation:</span> {tool.limitation}
                  </p>
                </div>
                <p className="mt-4 text-sm text-gray-300 leading-6">{tool.scenario}</p>
              </article>
            ))}
          </div>
        </section>
              ...
              <table className="w-full min-w-[760px] border-separate border-spacing-0 text-left text-sm">
                <thead>
                  <tr className="text-gray-400">
                    <th className="border-b border-gray-800 px-4 py-3 font-medium">Tool</th>
                    <th className="border-b border-gray-800 px-4 py-3 font-medium">Type</th>
                    <th className="border-b border-gray-800 px-4 py-3 font-medium">Local</th>
                    <th className="border-b border-gray-800 px-4 py-3 font-medium">Approach</th>
                    <th className="border-b border-gray-800 px-4 py-3 font-medium">Setup</th>
                  </tr>
                </thead>
                <tbody>
                  {comparisonRows.map((row) => (
                    <tr key={row[0]}>
                      {row.map((cell) => (
                        <td key={cell} className="border-b border-gray-900 px-4 py-3 text-gray-300">
                          {cell}
                        </td>
                      ))}
                    </tr>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-gray-800 bg-gray-950 p-5 sm:p-6">
              ...
              <ul className="mt-4 space-y-3 text-sm text-gray-300 leading-6">
                <li><span className="text-gray-100">OpenRedaction:</span> Node.js apps, pre-processing before OpenAI, deterministic output.</li>
                <li><span className="text-gray-100">Presidio:</span> Python stack, NLP detection, local execution.</li>
                <li><span className="text-gray-100">AWS / Google:</span> Managed environments, external processing acceptable, less setup.</li>
              </ul>
            </div>

            <div className="rounded-xl border border-gray-800 bg-gray-950 p-5 sm:p-6">
              <h2 className="text-2xl font-semibold">Example: redacting PII before sending to an API</h2>
              <div className="mt-4 rounded-lg border border-gray-800 bg-black p-4 text-sm text-gray-200">
                ...
                <div className="font-mono">Send this to john@email.com and call 555-123-4567</div>
              </div>
              <div className="mt-4 rounded-lg border border-gray-800 bg-black p-4 text-sm text-green-400">
                ...
                <div className="font-mono">Send this to [REDACTED] and call [REDACTED]</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
          <div className="rounded-xl border border-gray-800 bg-gray-950 p-5 sm:p-6">
            <h2 className="text-2xl font-semibold">Why open source redaction is growing</h2>
            <ul className="mt-4 space-y-3 text-sm text-gray-300 leading-6">
